...
def generate_hazard_table_from_config(config: dict, output_dir: Path, filename: str):
    ...
    # --- Apply COLA ---
    df['cola_pct'] = df['simulation_year'].map(cfg_cola['by_year']).fillna(0.02) # Default if year not in map
    ...
    df['term_rate'] = df['tenure_band'].map(tenure_term_multiplier_map) * cfg_term['base_rate_for_new_hire']
    ...
    final_columns = [
        "simulation_year", "employee_level", "tenure_band", "cfg",
        "term_rate", "promotion_rate", 
        "cola_pct", "merit_raise_pct", "promotion_raise_pct"
    ]
    df = df[final_columns]
