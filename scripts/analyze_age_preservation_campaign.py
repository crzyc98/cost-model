...
def load_starting_census_age_distribution():
    ...
    df["calculated_age"] = (
        (simulation_start - pd.to_datetime(df["employee_birth_date"])).dt.days / 365.25
    ).round(1)
...
def analyze_best_config_performance():
    ...
    print(
        f"  Estimated annual turnover: {annual_turnover_estimate:.3f} ({annual_turnover_estimate*100:.1f}%)"
    )
    ...
    for band in baseline_age_dist.keys():
        ...
        print(
            f"{band:>8} {baseline_prop:>10.3f} {achieved_prop:>10.3f} {difference:>+12.3f} {error:>8.3f}"
        )
    ...
    if new_hire_avg_age < census_avg_age:
        ...
        print(
            f"  - With {estimated_annual_hires:.1f} annual hires, this significantly impacts age mix"
        )
