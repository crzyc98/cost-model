...
"""
import argparse, sys, yaml, subprocess, logging
...
def sample_config(base_cfg, rng):
    ...
    sal = rng.normal(loc=base_cfg.get('annual_compensation_increase_rate',0.03),
                     scale=base_cfg.get('salary_growth_std',0.03*0.02))
    lower, upper = sal - 2*base_cfg.get('salary_growth_std',0.), sal + 2*base_cfg.get('salary_growth_std',0.)
    cfg['annual_compensation_increase_rate'] = float(np.clip(sal, lower, upper, out=np.empty_like(sal)))
    return cfg
...
def main():
    ...
    for i in range(1, args.runs+1):
        ...
        summary.append({
            ...
            'growth': growth,
            'plan_cost': df.get('PlanCost', pd.Series()).iloc[-1] if 'PlanCost' in df else None,
            'avg_deferral_pct': df.get('AvgDeferralPct', pd.Series()).iloc[-1] if 'AvgDeferralPct' in df else None
        })
