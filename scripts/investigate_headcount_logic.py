...
def extract_termination_analysis(
    events_df: pd.DataFrame, snapshots: List[pd.DataFrame]
) -> Dict[str, Any]:
...
def run_diagnostic_simulation(config: Dict[str, Any], config_name: str) -> Dict[str, Any]:
    ...
    logger.info(
        f"Key parameters - target_growth: {target_growth}, new_hire_rate: {new_hire_rate}, "
    ...
    try:
        ...
        for year_offset in range(projection_years):
            ...
            year_events, final_snapshot = run_one_year(
                ...
                plan_rules=(
                    config_ns.plan_rules.__dict__ if hasattr(config_ns, "plan_rules") else {}
                ),
        ...
        logger.info(
            f"Simulation completed successfully. Final headcount: {results['final_headcount']}"
        )
        return results
...
def main():
    ...
    try:
        ...
        print(
            f"New Hire Termination Rate: {baseline_results['parameters']['new_hire_termination_rate']}"
        )
        print()
        ...
        print(
            f"  Total Terminations: {baseline_results['termination_analysis'].get('total_terminations', 'N/A')}"
        )
        print()
        ...
        print(
            f"  Total Hires: {maintain_headcount_results['hiring_analysis'].get('total_hires', 'N/A')}"
        )
        print(
            f"  Total Terminations: {maintain_headcount_results['termination_analysis'].get('total_terminations', 'N/A')}"
        )
        print()
