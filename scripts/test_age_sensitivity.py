...
def analyze_age_distribution(output_dir: str, scenario_name: str) -> Dict:
    ...
    try:
        ...
        for year in sorted(years):
            ...
            if len(age_data) > 0:
                age_bands = year_data['employee_age_band'].value_counts().to_dict() if 'employee_age_band' in year_data.columns else {}
                age_stats[year] = {
                    'count': len(age_data),
    ...
    for result in valid_results:
        ...
        for year in result['years']:
            if age_stats[year]:
                ...
                print(f"{year:<6} {stats['count']:<8} {stats['mean_age']:<10.1f} {stats['median_age']:<12.1f} {bands_str}")
            else:
