...
from cost_model.state.schema import EMP_ACTIVE, EMP_GROSS_COMP, EMP_HIRE_DATE, EMP_ID
...
def test_exact_targeting_integration():
    ...
    print(
        f"- New hire termination rate: {year_context.global_params.attrition.new_hire_termination_rate:.1%}"
    )
    print()
    ...
    try:
        start_count, survivor_count, target_eoy, gross_hires, forced_terminations = (
            hiring_orchestrator._compute_targets(
        ...
        expected_gross = round(
            expected_net_needed
            / (1 - year_context.global_params.attrition.new_hire_termination_rate)
        )

        print("Verification:")
        print(
            f"- Expected target: {expected_target} (actual: {target_eoy}) ✓"
            if target_eoy == expected_target
            else f"- Expected target: {expected_target} (actual: {target_eoy}) ✗"
        )
        print(
            f"- Expected gross hires: {expected_gross} (actual: {gross_hires}) ✓"
            if gross_hires == expected_gross
            else f"- Expected gross hires: {expected_gross} (actual: {gross_hires}) ✗"
        )
        print(
            f"- Expected forced terms: 0 (actual: {forced_terminations}) ✓"
            if forced_terminations == 0
            else f"- Expected forced terms: 0 (actual: {forced_terminations}) ✗"
        )
        print()

        # Test new hire termination calculation
        expected_nh_terms = round(
            gross_hires * year_context.global_params.attrition.new_hire_termination_rate
        )
        expected_final_headcount = (
            survivor_count + gross_hires - expected_nh_terms - forced_terminations
        )
...
def test_forced_termination_scenario():
    ...
    try:
        start_count, survivor_count, target_eoy, gross_hires, forced_terminations = (
            hiring_orchestrator._compute_targets(
                snapshot=survivors_snapshot, year_context=year_context, start_count=len(snapshot)
            )
        )
        ...
        if forced_terminations > 0:
            print(
                f"✅ Forced termination logic is working! Need to terminate {forced_terminations} employees."
            )
        else:
