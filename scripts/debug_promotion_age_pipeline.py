...
def test_age_multiplier_application():
    ...
    from cost_model.state.schema import EMP_BIRTH_DATE, EMP_ID, EMP_LEVEL
    ...
    result = _apply_promotion_age_multipliers(
        test_snapshot, promotion_matrix, 2025, hazard_defaults
    )
    ...
    for _, row in result.iterrows():
        ...
        if age < 30 and multiplier != 2.5:
            logger.error(
                f"❌ Young employee {emp_id} (age {age}) should have multiplier 2.5, got {multiplier}"
            )
            return False
...
def test_markov_sampling_with_age():
    ...
    try:
        result = apply_promotion_markov(test_snapshot, rng=rng, simulation_year=2025)

        # Check for promotions
        promotions = (result[EMP_LEVEL] > test_snapshot[EMP_LEVEL]).sum()
        logger.info(
            f"   Promotions with 2.5x multiplier: {promotions}/{len(test_snapshot)} = {promotions/len(test_snapshot):.1%}"
        )
...
def test_full_promotion_pipeline():
    ...
    logger.info(
        f"   Young employees (<30): {(2025 - pd.to_datetime(test_snapshot[EMP_BIRTH_DATE]).dt.year < 30).sum()}"
    )
    ...
    try:
        promotions_df, raises_df, exits_df = apply_markov_promotions(
            snapshot=test_snapshot, promo_time=promo_time, rng=rng, simulation_year=2025
        )
        ...
        if len(promotions_df) > 0:
            ...
            young_promoted = (
                2025 - pd.to_datetime(promoted_employees[EMP_BIRTH_DATE]).dt.year < 30
            ).sum()
