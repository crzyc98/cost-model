...
def analyze_termination_events(events_df):
    ...
    if 'employee_hire_date' in term_events.columns and 'event_time' in term_events.columns:
        ...
        analysis.update({
            ...
            'new_hire_term_rate': len(new_hire_terms) / len(term_events) if len(term_events) > 0 else 0,
            'new_hire_terms_by_year': new_hire_terms['year'].value_counts().sort_index().to_dict(),
            'experienced_terms_by_year': experienced_terms['year'].value_counts().sort_index().to_dict(),
        })
...
def main():
    ...
    try:
        ...
        print(f"New Hire Termination Rate: {global_params.get('new_hire_termination_rate', 'unknown')}")
        ...
        if 'error' not in balance_analysis:
            for year, data in balance_analysis.items():
                print(f"Year {year}: {data['hires']} hires, {data['terminations']} terms, "
