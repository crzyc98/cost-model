...
SKOPT_SPACE = [
    Real(bounds[0], bounds[1], name=f"param_{i}") for i, (_, bounds) in enumerate(PARAM_BOUNDS)
]
...
def dict_to_namespace(d):
    ...
    if isinstance(d, dict):
        ...
        return SimpleNamespace(
            **{str(k): dict_to_namespace(v) for k, v in d.items() if isinstance(k, str)}
        )
...
def run_simulation_and_penalty(param_values, base_cfg, pid=0):
    ...
    hazard_table = build_hazard_table(years, snapshot, ns.global_parameters, plan_rules_ns)
    ...
    for year in range(
        ...
        event_log, snapshot = run_one_year(
            ...
            snapshot,
            year,
            SimpleNamespace(**{**ns.global_parameters.__dict__, "plan_rules": plan_rules_ns}),
            None,
            hazard_table,
    ...
    df["avg_growth"] = df.groupby(["status"])["avg_comp"].pct_change().fillna(0)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        choices=["grid", "bayes"],
        default="bayes",
        help="Sweep mode: grid or bayes (default: bayes)",
    )
    parser.add_argument(
        "--n_calls", type=int, default=60, help="Number of calls for Bayesian optimization"
    )
    ...
    def run_grid():
        ...
        grid = [
            dict(zip(SKOPT_PARAM_PATHS, vals))
            for vals in itertools.product(*[np.linspace(*b, 10) for _, b in PARAM_BOUNDS])
        ]
        records = []
        yaml_loader = YAML()
        with open(CONFIG_PATH, "r") as f:
        ...
        for pid, param_dict in enumerate(grid):
            ...
            penalty, best_param_dict, df = run_simulation_and_penalty(
                param_values, base_cfg, pid=pid
            )
    ...
    def run_bayes():
        ...
        with open(CONFIG_PATH, "r") as f:
            base_cfg = yaml_loader.load(f)
        res = gp_minimize(
            objective, SKOPT_SPACE, n_calls=args.n_calls, random_state=42, verbose=True
        )
        print("\nBest parameters:", res.x)
        print("Best penalty:", res.fun)
        penalty, best_param_dict, df = run_simulation_and_penalty(res.x, base_cfg, pid=0)
