...
from cost_model.state.tenure import TENURE_BAND_CATEGORICAL_DTYPE, assign_tenure_band
...
def extract_parameters(dev_tiny: Dict[str, Any], hazard_defaults: Dict[str, Any]) -> Dict[str, Any]:
    ...
    logger.info(
        f"Extracted parameters: start_year={params['start_year']}, "
...
def calculate_opening_headcount(params: Dict[str, Any]) -> Tuple[int, int]:
    ...
    logger.info(
        f"Expected year-end actives: {opening_active * existing_survival_rate + new_hires * new_hire_survival_rate:.1f}"
    )
...
def calculate_compensation(
    level_id: int, age: int, job_levels: List[Dict[str, Any]], rng: np.random.Generator
) -> float:
...
def generate_opening_snapshot(
    opening_active: int, params: Dict[str, Any], rng: np.random.Generator
) -> pd.DataFrame:
    ...
    base_ages = generate_ages(
        opening_active, 40, 10, params["min_working_age"], params["max_working_age"], rng
    )
    # Adjust ages based on tenure to make it realistic
    ages = np.clip(
        base_ages + tenure_years * 0.5, params["min_working_age"], params["max_working_age"]
    ).astype(int)
    ...
    compensations = [
        calculate_compensation(level, age, job_levels, rng) for level, age in zip(levels, ages)
    ]
    ...
    data = {
        ...
        EMP_DEFERRAL_RATE: rng.choice(
            [0.0, 0.03, 0.05, 0.06], size=opening_active, p=[0.4, 0.3, 0.2, 0.1]
        ),  # Simple deferral distribution
...
def apply_terminations_to_existing(
    df: pd.DataFrame, term_rate: float, start_year: int, rng: np.random.Generator
) -> pd.DataFrame:
    """Apply terminations to existing employees."""
    num_to_terminate = int(round(len(df) * term_rate))
    logger.info(
        f"Applying {num_to_terminate} terminations to existing employees (rate: {term_rate:.3f})"
    )
...
def generate_new_hires(
    num_new_hires: int, params: Dict[str, Any], existing_ids: List[str], rng: np.random.Generator
) -> pd.DataFrame:
    ...
    ages = generate_ages(
        num_new_hires,
        params["new_hire_average_age"],
        params["new_hire_age_std_dev"],
        params["min_working_age"],
        params["max_working_age"],
        rng,
    )
    ...
    tenure_years = [
        (datetime(start_year, 12, 31) - hire_date).days / 365.25 for hire_date in hire_dates
    ]
    ...
    data = {
        ...
        EMP_DEFERRAL_RATE: rng.choice(
            [0.0, 0.03, 0.05], size=num_new_hires, p=[0.6, 0.3, 0.1]
        ),  # Lower participation for new hires
...
def apply_terminations_to_new_hires(
    df: pd.DataFrame, term_rate: float, start_year: int, rng: np.random.Generator
) -> pd.DataFrame:
...
def add_derived_fields(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["SimYearStartDate"] = df.apply(
        lambda row: (
            row[EMP_HIRE_DATE]
            if row["WasNewHireInSimYear"]
            else datetime(row[SIMULATION_YEAR], 1, 1)
        ),
        axis=1,
    )
    ...
    df["TenureAtYearStart"] = df.apply(
        lambda row: (
            ...
            - (datetime(row[SIMULATION_YEAR], 12, 31) - datetime(row[SIMULATION_YEAR], 1, 1)).days
            / 365.25
        ),
        axis=1,
    )
    ...
    df["AgeAtYearStart"] = (
        df.apply(
            lambda row: (
                (datetime(row[SIMULATION_YEAR], 1, 1) - row[EMP_BIRTH_DATE]).days / 365.25
...
def print_summary_statistics(df: pd.DataFrame):
    ...
    # Verification
    assert (
        len(full_year_actives)
        + len(new_hire_actives)
        + len(new_hire_terms)
        + len(experienced_terms)
        == total_employees
    )
    ...
    try:
        ...
        output_df = all_employees_df[
            ...
            + [
                ...
                "TenureAtYearStart",
                "AgeAtYearStart",
                "CompensationAtYearStart",
                "CompensationAtYearEnd",
            ]
        ]
