...
def load_hr_metrics(snapshots_dir):
    ...
    for fname in sorted(os.listdir(snapshots_dir)):
        ...
        comp_field = 'employee_gross_compensation' if 'employee_gross_compensation' in df.columns else 'gross_compensation'
        records.append({
...
def main(snapshots_dir, plan_outputs_dir):
    ...
    merged['comp_match'] = (
        merged['comp_hr'].round(2)
        == merged['total_plan_year_compensation'].round(2)
    )
    ...
    if ok.all():
        ...
        print(merged.loc[
            ~ok,
            ['scenario','year','hc_hr','headcount','comp_hr','total_plan_year_compensation']
        ])
