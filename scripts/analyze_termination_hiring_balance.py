...
def load_validation_run_data(output_dir: str = "output_dev/projection_cli_results") -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    logger.info(f"Loaded {len(events_df)} events, {len(snapshots_df)} snapshot records, {len(summary_df)} summary records")
...
def analyze_terminations_by_tenure(events_df: pd.DataFrame, snapshots_df: pd.DataFrame) -> Dict[str, Any]:
    ...
    analysis = {
        ...
        'nh_terms_by_year': nh_terms['year'].value_counts().sort_index().to_dict() if not nh_terms.empty else {},
        'exp_terms_by_year': exp_terms['year'].value_counts().sort_index().to_dict() if not exp_terms.empty else {}
    }
...
def main():
    ...
    if headcount_analysis:
        ...
        for year, data in headcount_analysis.items():
            if year != 'summary':
                print(f"  Year {year}: {data['active_employees']} active, {data['terminated_employees']} terminated")
    ...
    if net_flow_analysis:
        ...
        for year, data in net_flow_analysis.items():
            if year != 'summary':
                print(f"  Year {year}: {data['hires']} hires, {data['total_terminations']} terms "
