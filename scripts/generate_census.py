...
try:
    ...
    def generate_employee_record(*args, **kwargs):
        raise NotImplementedError("generate_employee_record missing")

    def calculate_derived_fields(*args, **kwargs):
        raise NotImplementedError("calculate_derived_fields missing")

    def select_weighted_terminations(*args, **kwargs):
        raise NotImplementedError("select_weighted_terminations missing")
...
def create_dummy_census_files(
    ...
    role_compensation_params: Dict[str, Dict] = {
        ...
        "Manager": {
            ...
            "comp_increase_per_age_year": 600,
            "comp_increase_per_tenure_year": 1000,
            "comp_log_mean_factor": 1.0,
            "comp_spread_sigma": 0.25,
            "comp_min_salary": 60000,
        },
        "Executive": {
            "comp_base_salary": 250000,
            "comp_increase_per_age_year": 1500,
            "comp_increase_per_tenure_year": 3000,
            "comp_log_mean_factor": 1.1,
            "comp_spread_sigma": 0.35,
            "comp_min_salary": 120000,
        },
    },
    ...
    """
    logger.info(
        "Creating %d dummy census files from %d to %d",
        num_years,
        base_year - num_years + 1,
        base_year,
    )
    ...
    def _normalize_dist(dist: dict, name: str) -> dict:
        ...
        if not np.isclose(total_prob, 1.0):
            logger.warning(
                f"{name} distribution probabilities sum to {total_prob:.4f}. Normalizing."
            )
    ...
    for year in range(start_year, base_year + 1):
        ...
        if year == start_year:
            ...
            logger.info(
                "Initial population generated (%d rows).",
            ...
            logger.info(
                "Starting year %d with %d active employees from %d.",
                year,
                num_previous,
                year - 1,
            )
            ...
            if num_to_terminate > 0 and num_previous > 0:
                if num_to_terminate >= num_previous:
                    logger.warning(
                        f"Termination rate ({termination_rate:.1%}) results in terminating all {num_previous} employees."
                    )
                    indices_to_terminate = previous_population_df.index
                else:
                    logger.info(
                        "Simulating %d terminations from prior year pop (weighted by low tenure/comp)...",
                        num_to_terminate,
                    )
                    # Use the imported helper function
                    indices_to_terminate = select_weighted_terminations(
                        previous_population_df, num_to_terminate, rng
                    )

                # Assign termination date *within the current year* (YYYY)
                terminated_records_this_year = previous_population_df.loc[
                    indices_to_terminate
                ].copy()
            ...
            if not survivor_df.empty:
                ...
                increase_factor = rng.normal(
                    loc=1 + annual_comp_increase_mean,
            ...
            current_active_end_of_year_df = pd.concat(
                [survivor_df, new_hires_df], ignore_index=True
            )

            # Add back those terminated *during this year*
            if not terminated_records_this_year.empty:
                logger.info(
                    "Including %d terminated records in census for %d",
                    len(terminated_records_this_year),
                    year,
                )
                # Ensure columns align before concat
                cols_active = set(current_active_end_of_year_df.columns)
                set(terminated_records_this_year.columns)
                ...
                cols_to_align = [
                    col
                    for col in current_active_end_of_year_df.columns
                    if col in terminated_records_this_year.columns
                ]
            ...
            logger.info(
                "Total relevant population for %d census: %d employees.",
                year,
                len(current_census_population_df),
            )
        ...
        if not current_census_population_df.empty:
            ...
            for col in [EMP_BIRTH_DATE, EMP_HIRE_DATE, EMP_TERM_DATE]:
                if col in current_census_population_df.columns:
                    current_census_population_df[col] = pd.to_datetime(
                        current_census_population_df[col], errors="coerce"
                    )

            # Calculate derived fields on the COMBINED population using imported helper
            df_final_year = calculate_derived_fields(
                current_census_population_df,
                year,
                IRS_LIMITS,
                employer_nec_rate,
                employer_match_rate,
                employer_match_cap_deferral_perc,
            )
            ...
            df_final_year.to_csv(
                output_filename, index=False, date_format="%Y-%m-%d"
            )  # date_format might be redundant now
            generated_files.append(str(output_filename))
            logger.info(
                "Saved Combined Census: %s (%d rows)",
                output_filename,
                len(df_final_year),
            )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=int,
        default=2024,
        help="Final year of census data to generate",
    )
    p.add_argument(
        "--pop",
        type=int,
