...
def analyze_promotion_events(scenario_name, output_dir):
    ...
    try:
        ...
        promotion_summary = (
            promotion_events.groupby("year").size().reset_index(name="promotion_count")
        )
    ...
    try:
        ...
        if "employee_age_band" in df.columns:
            ...
            for year in sorted(df["year"].unique()):
                ...
                age_level_summary = (
                    year_data.groupby("employee_age_band")["employee_level"]
                    .agg(["count", "mean"])
                    .round(2)
                )
                ...
                for age_band, stats in age_level_summary.iterrows():
                    logger.info(
                        f"     {age_band}: {stats['count']} employees, avg level {stats['mean']}"
                    )
...
def compare_scenarios():
    ...
    baseline_promotions = (
        scenario_results.get("promotion_baseline", {})
        .get("promotions", {})
        .get("total_promotions", 0)
    )
    high_early_promotions = (
        scenario_results.get("promotion_high_early", {})
        .get("promotions", {})
        .get("total_promotions", 0)
    )
