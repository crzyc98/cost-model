...
def run_simulation_scenario(scenario_name, hazard_config_file, output_dir):
    ...
    try:
        result = subprocess.run(
            cmd, env=env, capture_output=True, text=True, timeout=300  # 5 minute timeout
        )
            ...
            for year in sorted(df["year"].unique()):
                ...
                if "employee_age_band" in year_data.columns:
                    ...
                    for age_band in year_data["employee_age_band"].unique():
                        ...
                        avg_level = (
                            age_group_data["employee_level"].mean() if total_employees > 0 else 0
                        )
...
def print_promotion_analysis(analysis_results):
        ...
        for year, year_data in scenario_data.items():
            ...
            if year_data:
                for age_band, metrics in year_data.items():
                    ...
                    logger.info(
                        f"    {age_band:<8}: {total:>3} employees, avg level {avg_level:.2f}"
                    )
            else:
    ...
    logger.info(
        f"{'Age Band':<10} {'Baseline':<10} {'High Early':<12} {'High Mid':<10} {'Low Late':<10}"
    )
    logger.info("-" * 60)
...
def main():
    ...
    for scenario_id, scenario_config in scenarios.items():
        success = run_simulation_scenario(
            scenario_config["name"], scenario_config["config"], scenario_config["output_dir"]
        )
