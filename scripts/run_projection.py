...
def aggregate_scenario_results(yearly_data: dict, scenario_config: dict) -> pd.DataFrame:
    ...
    for year, df in yearly_data.items():
        active = df[df.get('is_active', False) == True]
...
def combine_raw(name: str, data: dict, out_dir: Path):
    ...
    combined = pd.concat([df.assign(Year=yr) for yr, df in data.items()], ignore_index=True)
...
def main():
    ...
    census_df = load_and_clean_census(str(args.census), {'required': ['ssn','birth_date','hire_date','gross_compensation']})
