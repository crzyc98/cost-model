...
from cost_model.engines.markov_promotion import (
    _apply_promotion_age_multipliers,
    _load_hazard_defaults,
    apply_markov_promotions,
)
...
def create_test_data_with_ages():
    ...
    age_groups = pd.cut(
        ages, bins=[0, 30, 40, 50, 60, 100], labels=["<30", "30-39", "40-49", "50-59", "60+"]
    )
...
def test_age_sensitivity():
    ...
    df_with_multipliers = _apply_promotion_age_multipliers(
        df_with_multipliers, matrix, simulation_year, hazard_defaults
    )
    ...
    young_multipliers = df_with_multipliers[df_with_multipliers[EMP_ID].str.startswith("YOUNG")][
        "_promotion_age_multiplier"
    ].unique()
    middle_multipliers = df_with_multipliers[df_with_multipliers[EMP_ID].str.startswith("MIDDLE")][
        "_promotion_age_multiplier"
    ].unique()
    older_multipliers = df_with_multipliers[df_with_multipliers[EMP_ID].str.startswith("OLDER")][
        "_promotion_age_multiplier"
    ].unique()
    ...
    promotions_df, raises_df, exits_df = apply_markov_promotions(
        df, promo_time=promo_time, rng=rng, simulation_year=simulation_year, promotion_matrix=matrix
    )

    logger.info(
        f"Promotion results: {len(promotions_df)} promotions, {len(raises_df)} raises, {len(exits_df)} exits"
    )
    ...
    if not promotions_df.empty:
        ...
        if young_rate > middle_rate > older_rate:
            logger.info(
                "✓ Age sensitivity working correctly: younger employees have higher promotion rates"
            )
            return True
        else:
