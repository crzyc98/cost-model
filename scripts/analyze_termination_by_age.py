...
def analyze_terminations_by_age(scenario_dir, scenario_name):
    ...
    if 'employee_age_band' in snapshot.columns:
        ...
        for age_band in sorted(snapshot['employee_age_band'].unique()):
            ...
            band_terminated = len([emp for emp in band_employees['employee_id'] if emp in terminated_ids])
            ...
            if scenario_name == "High Early Attrition":
                expected_multipliers = {'<30': 2.5, '30-39': 1.8, '40-49': 1.0, '50-59': 0.7, '60-65': 0.5}
            elif scenario_name == "High Late Attrition":
                expected_multipliers = {'<30': 0.5, '30-39': 0.7, '40-49': 1.0, '50-59': 2.0, '60-65': 3.5}
            else:  # Baseline
                expected_multipliers = {'<30': 1.0, '30-39': 1.0, '40-49': 1.0, '50-59': 1.0, '60-65': 1.0}
            ...
            print(f"{age_band:<10} {band_total:<8} {band_terminated:<12} {band_rate:<7.1f}% {expected}x")
