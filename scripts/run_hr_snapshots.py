...
def main():
    ...
    if baseline is None:
        logger.error(f"ERROR: '{baseline_scenario_name}' scenario not found in config file {args.config}.")
        sys.exit(1)
    ...
    log_hr_params = {
        ...
        'annual_compensation_increase_rate': hr_cfg.get('annual_compensation_increase_rate'),
    ...
    try:
        ...
        if census_path.suffix.lower() == '.parquet':
            ...
            logger.info(f"Loaded {len(start_df)} records from Parquet census: {census_path}")
            ...
            for col in date_cols:
                if col in start_df.columns:
                    ...
                    if start_df[col].isnull().any():
                        logger.warning(f"Column '{col}' (loaded as {original_dtype}) contained values that could not be parsed as dates.")
            ...
            try:
                ...
                logger.info(f"Loaded {len(start_df)} records from CSV census: {census_path}")
                ...
                for col in parse_dates_present:
                     if pd.api.types.is_object_dtype(start_df[col]):
                         logger.warning(f"Column '{col}' in CSV could not be fully parsed as dates, check format.")
            ...
            logger.error(f"Unsupported census file format: {census_path}. Please use .csv or .parquet.")
            sys.exit(1)
        ...
        if source_id_column in start_df.columns:
            if target_id_column not in start_df.columns:
                logger.info(f"Renaming column '{source_id_column}' to '{target_id_column}' for consistency.")
                start_df.rename(columns={source_id_column: target_id_column}, inplace=True)
                logger.debug(f"Columns after rename: {start_df.columns.tolist()}")
            elif source_id_column != target_id_column:
                logger.warning(f"Both '{source_id_column}' and '{target_id_column}' found; using '{target_id_column}'.")
        elif target_id_column not in start_df.columns:
            logger.error(f"Required ID column '{source_id_column}' or '{target_id_column}' not found in census file. Aborting.")
            sys.exit(1)
    ...
    for year, df in hr_snapshots.items():
        ...
        if EMP_GROSS_COMP in df.columns:
             ...
             logger.warning(f"Column '{EMP_GROSS_COMP}' not found in snapshot for year {year}. Setting total compensation to 0 for summary.")
    ...
    if metrics:
        ...
        try:
            metrics_df.to_csv(summary_path, index=False, float_format='%.2f') # Format floats
            logger.info(f"Wrote HR summary: {summary_path}")
            logger.info(f"Summary includes {len(metrics_df)} years, covering {total_records_written} total yearly records.")
        except Exception as e:
