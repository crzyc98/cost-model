...
def main():
    ...
    scenarios = [
        (
            "Baseline (1.0x all)",
            "output_dev/promotion_age_test/baseline/P-Baseline (All 1.0)_final_cumulative_event_log.parquet",
        ),
        (
            "High Early (2.5x <30)",
            "output_dev/promotion_age_test/high_early/P-High Early (<30 = 2.5x)_final_cumulative_event_log.parquet",
        ),
        (
            "High Mid (2.5x 30-39)",
            "output_dev/promotion_age_test/high_mid/P-High Mid (30-39 = 2.5x)_final_cumulative_event_log.parquet",
        ),
        (
            "Low Late (0.1x 50+)",
            "output_dev/promotion_age_test/low_late/P-Low Late (50+ = 0.1x)_final_cumulative_event_log.parquet",
        ),
    ]
