...
try:
    from cost_model.state.schema import EMP_LEVEL, EMP_ID, EMP_HIRE_DATE, EMP_EXITED
    logger.info(f"Successfully imported schema constants: {EMP_LEVEL=}, {EMP_ID=}, {EMP_HIRE_DATE=}, {EMP_EXITED=}")
except ImportError:
    ...
    logger.info(f"Using fallback schema constants: {EMP_LEVEL=}, {EMP_ID=}, {EMP_HIRE_DATE=}, {EMP_EXITED=}")
...
def create_test_data():
    ...
    df = pd.DataFrame({
        ...
        EMP_HIRE_DATE: [today - pd.Timedelta(days=np.random.randint(30, 1000)) for _ in range(100)],
