...
def run_enhanced_hiring_test():
    ...
    try:
        ...
        logger.info(
            f"  NH Termination Rate: {main_config.global_parameters.new_hire_termination_rate:.1%}"
        )
        ...
        if snapshots_path.exists() and events_path.exists():
            ...
            analyze_headcount_results(
                snapshots_df, events_df, main_config.global_parameters.target_growth
            )
...
def analyze_headcount_results(
    snapshots_df: pd.DataFrame, events_df: pd.DataFrame, target_growth: float
):
    ...
    logger.info(
        f"  Growth Target Met: {'✅' if growth_success else '❌'} "
        f"({annual_growth:.1%} vs {target_growth:.1%})"
    )
    logger.info(
        f"  NH Rate Accurate: {'✅' if nh_rate_success else '❌'} "
        f"({actual_nh_rate:.1%} vs 25.0%)"
        if total_hires > 0
        else "  NH Rate: N/A"
    )
    ...
    if overall_success:
        ...
        logger.info(
            "The hiring algorithm now correctly accounts for expected experienced attrition."
        )
    else:
