...
def load_config_parameters(config_path: Path) -> dict:
    ...
    params = {
        ...
        "new_hire_termination_rate": config["global_parameters"]["attrition"].get(
...
def analyze_yearly_events(output_dir: Path, params: dict) -> dict:
    ...
    for year_offset in range(projection_years):
        ...
        # Analyze events
        hire_events = (
            events_df[events_df["event_type"] == EVT_HIRE]
            if not events_df.empty
            else pd.DataFrame()
        )
        term_events = (
            events_df[events_df["event_type"] == EVT_TERM]
            if not events_df.empty
            else pd.DataFrame()
        )
        ...
        active_count = (
            snapshot_df[EMP_ACTIVE].sum() if EMP_ACTIVE in snapshot_df.columns else len(snapshot_df)
        )
...
def calculate_actual_vs_expected(yearly_results: dict, params: dict) -> dict:
    ...
    for year in sorted(yearly_results.keys()):
        ...
        actual_nh_term_rate = (
            year_data["new_hire_terminations"] / year_data["total_hires"]
            if year_data["total_hires"] > 0
            else 0.0
        )
...
def calculate_growth_analysis(yearly_results: dict, params: dict) -> dict:
    ...
    total_nh_terms = sum(
        year_data["new_hire_terminations"] for year_data in yearly_results.values()
    )
...
def identify_discrepancies(yearly_results: dict, growth_analysis: dict, params: dict):
    ...
    expected_net = (
        growth_analysis["expected_final_count"] - growth_analysis["estimated_initial_count"]
    )
...
def main():
    """Main analysis function for Campaign 6 best configuration."""

    # Configuration paths
    config_path = Path("campaign_6_results/best_config.yaml")
    output_dir = Path("tuned/output_config_099_20250605_122848/Baseline")
    ...
    print("Campaign 6 Best Configuration Analysis")
    print("=" * 50)
    print()
    ...
    results = {
        ...
        "yearly_results": {
            k: {key: val for key, val in v.items() if key not in ["hire_events", "term_events"]}
    ...
    output_file = Path("campaign6_hiring_termination_analysis.json")
