...
from .columns import ColumnGroups, EventColumns, SnapshotColumns
from .dtypes import DataTypeValidator, EventDTypes, SnapshotDTypes
...
class SchemaValidator:
    ...
    def validate_snapshot_schema(
        self, df: pd.DataFrame, check_data_quality: bool = True
    ) -> ValidationResult:
        ...
        self._validate_required_columns(df, ColumnGroups.REQUIRED_SNAPSHOT, result)
    ...
    def validate_event_schema(
        ...
        # Required columns validation
        self._validate_required_columns(df, ColumnGroups.EVENT_CORE, result)
    ...
    def _validate_basic_structure(
        self, df: pd.DataFrame, schema_type: str, result: ValidationResult
    ):
    ...
    def _validate_required_columns(
        self, df: pd.DataFrame, required_columns: List[str], result: ValidationResult
    ):
    ...
    def _validate_snapshot_dtypes(self, df: pd.DataFrame, result: ValidationResult):
    ...
    def _validate_event_dtypes(self, df: pd.DataFrame, result: ValidationResult):
    ...
    def _validate_snapshot_data_quality(self, df: pd.DataFrame, result: ValidationResult):
        ...
        if SnapshotColumns.EMP_GROSS_COMP in df.columns:
            ...
            if negative_comp.any():
                result.add_warning(
                    f"Found {negative_comp.sum()} employees with negative compensation"
                )
            ...
            if very_high_comp.any():
                result.add_warning(
                    f"Found {very_high_comp.sum()} employees with very high compensation (>$1M)"
                )
    ...
    def _validate_event_data_quality(self, df: pd.DataFrame, result: ValidationResult):
    ...
    def _validate_snapshot_business_rules(self, df: pd.DataFrame, result: ValidationResult):
        """Validate business rules for snapshots."""
        # Active employees should not have termination dates
        if all(
            col in df.columns for col in [SnapshotColumns.EMP_ACTIVE, SnapshotColumns.EMP_TERM_DATE]
        ):
            active_with_term_date = (df[SnapshotColumns.EMP_ACTIVE] == True) & df[
        ...
        if all(
            ...
            inactive_without_term_date = (df[SnapshotColumns.EMP_ACTIVE] == False) & df[
                SnapshotColumns.EMP_TERM_DATE
            ].isna()
        ...
        if SnapshotColumns.EMP_AGE in df.columns:
            unreasonable_age = (df[SnapshotColumns.EMP_AGE] < 16) | (
                df[SnapshotColumns.EMP_AGE] > 100
            )
            if unreasonable_age.any():
                result.add_warning(
                    f"Found {unreasonable_age.sum()} employees with unreasonable ages"
                )

    def _validate_event_business_rules(self, df: pd.DataFrame, result: ValidationResult):
        ...
        for col in rate_columns:
            if col in df.columns:
                ...
                if invalid_rates.any():
                    result.add_warning(
                        f"Found {invalid_rates.sum()} invalid rates in {col} (outside 0-1 range)"
                    )
    ...
    def _validate_event_type_rules(
        self, event_type: str, events: pd.DataFrame, result: ValidationResult
    ):
...
def validate_snapshot_schema(
    df: pd.DataFrame, strict_mode: bool = False, check_data_quality: bool = True
) -> ValidationResult:
