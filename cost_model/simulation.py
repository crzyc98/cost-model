...
from cost_model.config.models import GlobalParameters, MainConfig  # Needed for type hints
...
try:
    ...
    def read_census_data(*args, **kwargs):
        raise ImportError("read_census_data missing")

    def write_snapshots(*args, **kwargs):
        raise ImportError("write_snapshots missing")

    def write_summary_metrics(*args, **kwargs):
        raise ImportError("write_summary_metrics missing")
...
try:
    ...
    def run_dynamics_for_year(*args, **kwargs):
        raise ImportError("run_dynamics_for_year missing")

    def apply_rules_for_year(*args, **kwargs):
        raise ImportError("apply_rules_for_year missing")
...
def run_simulation(
    ...
    try:
        ...
        scenario_output_name = (
            main_config.scenarios[scenario_name].name or scenario_name
        )  # Use defined name or key
    except Exception:
        logger.exception(
            f"Error extracting key parameters from scenario config for '{scenario_name}'."
        )
        return
    ...
    try:
        ...
        try:
            ...
            if hazard.empty:
                ...
                hazard = hazard.rename(
                    columns={k: v for k, v in column_mapping.items() if k in hazard.columns}
                )
        except Exception as fallback_error:
            logger.error(f"Fallback hazard table loading also failed: {fallback_error}")
            raise RuntimeError(
                "Both dynamic and static hazard table loading failed"
            ) from fallback_error
    ...
    for i in range(projection_years):
        ...
        logger.debug(
            f"Applying age and age band calculations for year {year} as of {as_of_date_for_age}"
        )
        ...
        if EMP_AGE_BAND in snapshot_df.columns:
            logger.debug(
                f"Sample EMP_AGE_BAND for year {year}: {snapshot_df[EMP_AGE_BAND].head().tolist()}"
            )
        ...
        year_events = (
            all_events[all_events["simulation_year"] == year]
            if "simulation_year" in all_events.columns
            else all_events
        )
        ...
        logger.info(
            f"Saved enhanced outputs for year {year} with {len(enhanced_yearly_snapshot)} employees"
        )
    ...
    if save_summary_metrics and summary_metrics_df is not None:
        try:
            ...
            summary_path = write_summary_metrics(
                summary_metrics_df, scenario_output_dir, output_prefix
            )
