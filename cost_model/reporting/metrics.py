...
def calculate_summary_metrics(
    ...
    logger.info(f"Calculating summary metrics for {len(all_results_df)} total records across years...")

    # --- Ensure correct data types ---
    all_results_df['employee_birth_date'] = pd.to_datetime(all_results_df['employee_birth_date'], errors='coerce')
    ...
    # --- Feature Engineering ---
    all_results_df['age'] = all_results_df['simulation_year'] - all_results_df['employee_birth_date'].dt.year
    all_results_df['is_participant'] = all_results_df['is_eligible'] & (all_results_df['employee_deferral_rate'] > 0)

    # Row-level ER and total compensation
    all_results_df['total_er_contribution'] = (
        all_results_df['employer_match_contribution'] + all_results_df['employer_core_contribution']
    )
    all_results_df['total_compensation'] = (
        all_results_df['employee_gross_compensation'] + all_results_df['employee_contribution'] + all_results_df['total_er_contribution']
    )

    # --- Group and Aggregate ---
    summary_metrics = all_results_df.groupby('simulation_year').agg(
    ...
    if not participants_df.empty:
        avg_deferral_rate = participants_df.groupby('simulation_year')['employee_deferral_rate'].mean().reset_index()
