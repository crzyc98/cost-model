...
try:
    ...
    EMP_HIRE_DATE, EMP_TERM_DATE, EMP_BIRTH_DATE = (
        "employee_hire_date",
...
def read_census_data(file_path: Path) -> Optional[pd.DataFrame]:
    ...
    try:
        ...
        if file_suffix == ".parquet":
            ...
            for col in date_cols_to_parse:
                if col in df.columns:
                    if not pd.api.types.is_datetime64_any_dtype(df[col]):
                        logger.debug(
                            f"Converting column '{col}' to datetime (read as {df[col].dtype})."
                        )
                        df[col] = pd.to_datetime(df[col], errors="coerce")
                        if df[col].isnull().any():
                            logger.warning(
                                f"Column '{col}' contained values that could not be parsed as dates during Parquet load."
                            )
            ...
            try:
                ...
                for col in parse_dates_present:
                    if pd.api.types.is_object_dtype(df[col]):
                        logger.warning(
                            f"Column '{col}' in CSV might not have been fully parsed as dates (still object type). Check format or consider errors='coerce'."
                        )
            ...
            logger.error(
                f"Unsupported census file format: {file_path}. Please use .csv or .parquet."
            )
        ...
        if id_col_found:
            if id_col_found != "employee_id":
                if "employee_id" in df.columns:
                    logger.warning(
                        f"Both '{id_col_found}' and 'employee_id' columns exist. Using 'employee_id'."
                    )
        ...
        if df["employee_id"].duplicated().any():
            logger.warning(
                "Duplicate values found in the 'employee_id' column. Ensure IDs are unique."
            )

        logger.info("Census data read and prepared successfully.")
        return df
    ...
    dummy_data = pd.DataFrame(
        {
            ...
            "employee_birth_date": pd.to_datetime(
                ["1990-01-15", "1985-05-20", "1992-11-30", "1988-07-01", "1995-03-10"]
            ),
            "employee_hire_date": pd.to_datetime(
                ["2015-06-01", "2010-08-15", "2018-01-20", "2012-09-01", "2020-04-15"]
            ),
            "employee_termination_date": [
                pd.NaT,
                pd.NaT,
