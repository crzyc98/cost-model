...
try:
    ...
    from cost_model.utils.columns import (
         EMP_BIRTH_DATE, EMP_HIRE_DATE, EMP_TERM_DATE # Example relevant date columns
         # Add other column constants if needed for output formatting/selection
    )
except ImportError:
    print("Warning (writers.py): Could not import column constants from utils. Using string literals.")
    EMP_BIRTH_DATE, EMP_HIRE_DATE, EMP_TERM_DATE = 'employee_birth_date', 'employee_hire_date', 'employee_termination_date'
...
def write_snapshots(
    ...
    for year, df in yearly_snapshots.items():
        ...
        sim_year = df['simulation_year'].iloc[0] if 'simulation_year' in df.columns else year
        out_path = output_dir / f"{file_prefix}_year{sim_year}.parquet"
        logger.debug(f"Writing snapshot for year {sim_year} to {out_path} ({len(df)} records)")
        try:
