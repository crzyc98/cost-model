...
class EventConsolidator(BaseProcessor):
    ...
    def consolidate_events(
        ...
        **kwargs
        ...
        try:
            ...
            result.add_metadata('event_types_included', [et for et, df in event_dataframes if not df.empty])
    ...
    def _standardize_event_format(self, events: pd.DataFrame, year: int) -> pd.DataFrame:
        ...
        for col in EVENT_COLS:
            if col not in standardized.columns:
                if col == SIMULATION_YEAR:
                    ...
                    elif any(numeric_word in col.lower() for numeric_word in ['amount', 'rate', 'value']):
        ...
        self.logger.info(f"Standardized {len(standardized)} events with {len(final_columns)} columns")
    ...
    def _validate_event_log(self, events: pd.DataFrame) -> Dict[str, Any]:
        ...
        if EMP_ID in events.columns:
            ...
            if null_emp_ids > 0:
                validation_result['warnings'].append(f"Found {null_emp_ids} events with null employee IDs")
        ...
        if EVENT_TIME in events.columns:
            ...
            if invalid_times > 0:
                validation_result['warnings'].append(f"Found {invalid_times} events with invalid times")
