...
class TerminationProcessor(BaseProcessor):
    ...
    def process_experienced_terminations(
        ...
        **kwargs,
        ...
        try:
            ...
            self.logger.info(
                f"Processing terminations for {len(experienced_employees)} experienced employees"
            )
            ...
            if not termination_events.empty:
                self.logger.info(
                    f"Generated {len(termination_events)} experienced termination events"
                )
            ...
            self.logger.error(
                f"Error during experienced termination processing: {e}", exc_info=True
            )
    ...
    def process_new_hire_terminations(
        ...
        **kwargs,
        ...
        """
        self.log_step_start(
            "Deterministic new-hire terminations", snapshot_size=len(snapshot), year=year
        )
        ...
        try:
            ...
            if not nh_termination_events.empty:
                self.logger.info(
                    f"Generated {len(nh_termination_events)} new hire termination events"
                )
    ...
    def validate_termination_inputs(
    ...
    def validate_new_hire_inputs(
        self, snapshot: pd.DataFrame, year: int, global_params: Any
    ) -> bool:
