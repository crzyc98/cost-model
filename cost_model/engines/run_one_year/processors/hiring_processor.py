...
class HiringProcessor(BaseProcessor):
    ...
    def process(
        ...
        **kwargs,
        ...
        """
        self.log_step_start("Generate/apply hires", snapshot_size=len(snapshot), year=year)
        ...
        self.log_step_end(
            "Generate/apply hires", events_generated=len(result.events), success=result.success
        )

        return result

    def _compute_headcount_targets(
        ...
        try:
            ...
            headcount_info = compute_headcount_targets(
                snapshot=snapshot, simulation_year=year, global_params=global_params
            )

            # Log headcount details
            current_headcount = (
                len(snapshot[snapshot.get(EMP_ACTIVE, True)])
