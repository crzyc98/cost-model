...
class DiagnosticLogger(BaseProcessor):
        ...
        **additional_context,
        ...
        try:
            ...
            self.diagnostic_data[f"{step_name}_{year}"] = {
                ...
                "active_before": (
                    snapshot_before[EMP_ACTIVE].sum()
                    if EMP_ACTIVE in snapshot_before.columns
                    else None
                ),
                "active_after": (
                    snapshot_after[EMP_ACTIVE].sum()
                    if EMP_ACTIVE in snapshot_after.columns
                    else None
                ),
            ...
            if disappeared:
                ...
                if not events.empty and EMP_ID in events.columns:
                    ...
                    if unaccounted_disappearances:
                        self.logger.warning(
                            f"  {step_name}: {len(unaccounted_disappearances)} employees disappeared without events"
                        )
                else:
            ...
            if appeared:
                ...
                if not events.empty and EMP_ID in events.columns:
                    ...
                    if unaccounted_appearances:
                        self.logger.warning(
                            f"  {step_name}: {len(unaccounted_appearances)} employees appeared without events"
                        )
                else:
    ...
    def log_year_summary(
        self, year: int, final_snapshot: pd.DataFrame, total_events: pd.DataFrame
    ) -> ProcessorResult:
        """
        ...
        try:
            ...
            self.diagnostic_data[summary_key] = {
                ...
                "active_employees": (
                    final_snapshot[EMP_ACTIVE].sum()
                    if EMP_ACTIVE in final_snapshot.columns
                    else None
                ),
                "total_events": total_events_count,
                "event_breakdown": (
                    total_events["event_type"].value_counts().to_dict()
                    if not total_events.empty and "event_type" in total_events.columns
                    else {}
                ),
            }
