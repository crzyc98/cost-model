...
def compute_hire_counts(
    ...
    logger.info(f"[RUN_ONE_YEAR YR={year}] Using new_hire_rate={new_hire_rate} from global_params.new_hire_rate")
    
    # Determine new hire termination rate
    new_hire_term_rate = getattr(global_params, "new_hire_termination_rate", 0.0)
    logger.info(f"[RUN_ONE_YEAR YR={year}] Using new_hire_termination_rate={new_hire_term_rate} from global_params.new_hire_termination_rate")
...
def sample_and_build_hire_comp_events(
    ...
    new_hires = sample_mixed_new_hires(
        level_counts, 
        age_range=(25, 55), 
        random_state=year_rng
    )
    
    # Rename columns to match schema
    new_hires = new_hires.rename(columns={
        'level_id': EMP_LEVEL, 
