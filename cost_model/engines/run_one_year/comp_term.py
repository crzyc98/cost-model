...
from cost_model.state.schema import EMP_ACTIVE as ACTIVE
...
def apply_compensation_and_terminations(
    ...
    logger.info(
        f"[YR={year}] SOY experienced mask: {experienced_count} | new hire mask: 0 | total: {len(snapshot)}"
    )
    ...
    if experienced_count > 0:
        ...
        if term_events_list:
            ...
            if term_ids:
                ...
                snapshot.loc[snapshot[EMP_ID].isin(term_ids), [ACTIVE, EMP_TERM_DATE]] = [
                    False,
            ...
            logger.info(
                f"[YR={year}] Post-term snapshot: {len(snapshot)} unique EMP_IDs, {snapshot.shape[0]} rows"
            )
            ...
            logger.info(
                f"[RUN_ONE_YEAR YR={year}] Terminations: {term_count}, survivors active: {active_count}"
            )
    ...
    if experienced_count > 0:
        ...
        logger.debug(
            f"[YR={year}] Experienced slice: {len(experienced_df)} rows; sample IDs: {experienced_df[EMP_ID].head().tolist()}"
        )
        ...
        nh_term_rate = (
            hazard_slice["new_hire_termination_rate"].mean()
            if "new_hire_termination_rate" in hazard_slice
            else 0.0
        )
        logger.info(
            f"[RUN_ONE_YEAR YR={year}] Rates → term={term_rate:.3f}, comp={mean_comp_rate:.3f}, nh_term={nh_term_rate:.3f}"
        )
        ...
        if experienced_df[EMP_GROSS_COMP].isna().any():
            logger.warning(
                f"[YR={year}] Found {experienced_df[EMP_GROSS_COMP].isna().sum()} NA values in {EMP_GROSS_COMP}, filling with 0"
            )
        ...
        for _, row in experienced_df.iterrows():
            ...
            if emp_id in comp_cols_before[EMP_ID].values:
                before_comp = comp_cols_before.loc[
                    comp_cols_before[EMP_ID] == emp_id, EMP_GROSS_COMP
                ].iloc[
                    0
