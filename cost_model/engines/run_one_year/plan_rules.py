...
from cost_model.plan_rules import contributions, eligibility, enrollment
...
def run_all_plan_rules(
    ...
    eligibility_cfg = type(
        "EligibilityConfig", (), {"min_age": min_age, "min_service_months": min_service_months}
    )()

    # Run eligibility checks - this returns a list of DataFrames with eligibility events
    eligibility_events = eligibility.run(snapshot=prev_snapshot, as_of=as_of, cfg=eligibility_cfg)
    ...
    if not eligibility_events_df.empty:
        ...
        enrollment_events_df = (
            pd.DataFrame(enrollment_events) if enrollment_events else pd.DataFrame()
        )
        ...
        if not enrollment_events_df.empty:
            ...
            enrollment_df = enrollment.run(
                snapshot=prev_snapshot, events=enrollment_events_df, as_of=as_of, cfg=enrollment_cfg
            )
    ...
    if isinstance(enrollment_df, list):
        ...
        events_df = (
            ...
            else pd.DataFrame(columns=[EMP_ID, "event_type", "event_time"])
        )
    ...
    proactive_decrease_df = (
        pd.DataFrame(columns=contrib_events.columns) if not contrib_events.empty else contrib_events
    )
