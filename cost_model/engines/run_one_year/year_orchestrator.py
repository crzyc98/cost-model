...
class YearOrchestrator:
    ...
    def run_one_year(
        ...
        **kwargs
        ...
        try:
            ...
            self._update_snapshot_with_events([promotion_result.events, termination_result.events], year)
            ...
            if self.diagnostic_logger:
                self.diagnostic_logger.log_year_summary(year, final_snapshot, event_consolidation_result.data)
    ...
    def _process_promotions(self, year: int, global_params: Any, hazard_table: pd.DataFrame,
                          rng: Any, rng_seed_offset: int):
    ...
    def _process_experienced_terminations(self, year: int, global_params: Any, hazard_table: pd.DataFrame,
                                        rng: Any, rng_seed_offset: int, deterministic_term: bool):
        ...
        if self.diagnostic_logger:
            self.diagnostic_logger.log_step_diagnostics(
                "experienced_terminations", snapshot_before, self.current_snapshot, result.events, year
            )
        
        return result
    
    def _process_hiring(self, year: int, global_params: Any, rng: Any,
                       census_template_path: Optional[str], rng_seed_offset: int):
    ...
    def _process_new_hire_terminations(self, year: int, global_params: Any, rng: Any, rng_seed_offset: int):
    ...
    def _consolidate_events(self, promotion_events: pd.DataFrame, termination_events: pd.DataFrame,
                          hiring_events: pd.DataFrame, contribution_events: pd.DataFrame,
                          nh_termination_events: pd.DataFrame, year: int):
    ...
    def _build_event_log(self, prev_event_log: pd.DataFrame, year_events: pd.DataFrame) -> pd.DataFrame:
        ...
        logger.info(f"Updated event log: {len(prev_event_log)} + {len(year_events)} = {len(updated_event_log)} events")
