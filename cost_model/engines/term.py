...
def run(
    ...
    if missing_rates.any():
        ...
        level2_01yr_mask = (merged_df[EMP_LEVEL] == 2) & (merged_df[EMP_TENURE_BAND] == '<1') & missing_rates
        ...
        if level2_01yr_count > 0:
            ...
            if not level2_rates.empty:
                ...
                if not level1_01yr.empty:
                    ...
                    logger.info(f"[TERM] Year {year}: Using level 1, <1 rate {fallback_rate:.4f} as proxy for level 2, <1")
                else:
