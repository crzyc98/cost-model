...
def bump(
    ...
    active = snapshot[snapshot[EMP_TERM_DATE].isna() | (snapshot[EMP_TERM_DATE] > as_of)].copy()
    ...
    merit_events = _generate_merit_events(
        active=active, hazard_slice=hazard_slice, as_of=as_of, year=year
    )

    # 4) Generate COLA events LAST (applies to everyone after merit raises)
    cola_events = generate_cola_events(
        snapshot=snapshot, hazard_slice=hazard_slice, as_of=as_of, rng=rng
    )
...
def _generate_merit_events(
    active: pd.DataFrame, hazard_slice: pd.DataFrame, as_of: pd.Timestamp, year: int
) -> List[pd.DataFrame]:
    """
    ...
    if "merit_raise_pct" in hazard_slice.columns:
        ...
        logger.warning(
            "[COMP] No merit raise column found in hazard slice. Returning empty events."
        )
    ...
    if level_col not in hazard_slice.columns:
        ...
        if possible_level_cols:
            ...
            raise KeyError(
                f"No level column found in hazard_slice. Available columns: {hazard_slice.columns.tolist()}"
            )
    ...
    if tenure_col not in hazard_slice.columns:
        ...
        if possible_tenure_cols:
            ...
            raise KeyError(
                f"No tenure band column found in hazard_slice. Available columns: {hazard_slice.columns.tolist()}"
            )

    logger.debug(
        f"[COMP] Using column '{level_col}' as EMP_LEVEL merge key and '{tenure_col}' as EMP_TENURE_BAND merge key; hazard rows={len(hazard_slice)}"
    )

    # Prepare hazard slice for merge
    hz = hazard_slice[[level_col, tenure_col, merit_col]].rename(
        columns={level_col: EMP_LEVEL, tenure_col: EMP_TENURE_BAND}
    )
    ...
    if len(hz_dedup) < len(hz):
        logger.debug(
            f"[COMP] Removed {len(hz) - len(hz_dedup)} duplicate (level, tenure_band) combinations in hazard slice for year {year}"
        )

    # Merge active employees with hazard data
    df = active.merge(hz_dedup, on=[EMP_LEVEL, EMP_TENURE_BAND], how="left").fillna({merit_col: 0})

    # Only process employees with positive merit raises
    excluded = active[~active[EMP_ID].isin(df[EMP_ID])]
    if not excluded.empty:
        logger.warning(
            f"[COMP.MERIT] {len(excluded)} active employees excluded from merit bump due to missing hazard table match. EMP_IDs: {excluded[EMP_ID].tolist()}"
        )
    ...
    df["meta"] = df.apply(
        lambda row: f"Merit raise for {row[EMP_ID]}: {row['old_comp']} -> {row['new_comp']} (+{row[merit_col]*100:.2f}%)",
        axis=1,
    )
...
def extract_promotion_raise_config_from_hazard(hazard_slice: pd.DataFrame) -> dict:
    ...
    if "promotion_raise_pct" not in hazard_slice.columns:
        logger.warning(
            "[COMP.PROMOTION] No promotion_raise_pct column in hazard slice. Using default config."
        )
        return {}
    ...
    if level_col not in hazard_slice.columns:
        ...
        if possible_level_cols:
            ...
            logger.warning(
                "[COMP.PROMOTION] No level column found in hazard slice. Using default config."
            )
            return {}
    ...
    logger.info(
        f"[COMP.PROMOTION] Extracted promotion raise config from hazard table: {len(promo_config)} mappings"
    )
    return promo_config
...
def generate_cola_events(
    ...
    """
    logger.info(
        f"[COMP.COLA] Generating COLA events for year {int(hazard_slice['simulation_year'].iloc[0])}"
    )
    ...
    try:
        ...
        if "cola_pct" not in hazard_slice.columns:
            logger.warning(
                "[COMP.COLA] No cola_pct column in hazard_slice. Returning empty events."
            )
            return [pd.DataFrame(columns=EVENT_COLS)]

        cola_pct = float(
            hazard_slice["cola_pct"].max()
        )  # Use max to ensure we get non-zero COLA if it exists
        year = int(hazard_slice["simulation_year"].iloc[0])

        # Create a simple hazard slice for the cola() function
        simple_hazard_slice = pd.DataFrame({"simulation_year": [year], "cola_pct": [cola_pct]})
        ...
        if cola_events and len(cola_events) > 0 and not cola_events[0].empty:
            ...
            logger.info(
                f"[COMP.COLA] Generated {len(events_df)} COLA events with rate {cola_pct:.1%}"
            )
        else:
