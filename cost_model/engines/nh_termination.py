...
def run_new_hires(
    ...
    if EMP_TENURE_BAND in df_nh.columns:
        ...
        logger.info(
            f"[NH-TERM] Original new hire tenure bands: {df_nh[EMP_TENURE_BAND].unique().tolist()}"
        )

        # Apply standardization
        df_nh[EMP_TENURE_BAND] = df_nh[EMP_TENURE_BAND].apply(standardize_tenure_band)
        logger.info(
            f"[NH-TERM] Standardized new hire tenure bands: {df_nh[EMP_TENURE_BAND].unique().tolist()}"
        )
    ...
    if EMP_TENURE_BAND in hz_slice_copy.columns:
        hz_slice_copy[EMP_TENURE_BAND] = hz_slice_copy[EMP_TENURE_BAND].apply(
            standardize_tenure_band
        )
        logger.info(
            f"[NH-TERM] Standardized hazard slice tenure bands: {hz_slice_copy[EMP_TENURE_BAND].unique().tolist()}"
        )

    # Get termination rate - CRITICAL FIX: Use NEW_HIRE_TERMINATION_RATE instead of NEW_HIRE_TERM_RATE
    nh_term_rate = (
        hz_slice_copy[NEW_HIRE_TERMINATION_RATE].iloc[0]
    ...
    if NEW_HIRE_TERMINATION_RATE in hz_slice_copy.columns:
        logger.info(
            f"[NH-TERM] Year {year}: Successfully sourced new_hire_termination_rate = {nh_term_rate}"
        )
    else:
        logger.warning(
            f"[NH-TERM] Year {year}: Column '{NEW_HIRE_TERMINATION_RATE}' NOT FOUND in hazard slice, using fallback rate = {nh_term_rate}"
        )
    logger.info(
        f"[NH-TERM] Year {year}: Calculated {k} new hires to terminate ({k}/{n} = {k/n*100:.1f}% if n>0 else 0%)"
    )
    ...
    for i, row in selected_df.iterrows():
        try:
            ...
            prorated = (
                comp * (days_worked / 365.25) if comp is not None and not pd.isna(comp) else None
            )
            ...
            logger.error(
                f"Error processing termination for employee {emp if 'emp' in locals() else 'unknown'}: {str(e)}"
            )
            continue
