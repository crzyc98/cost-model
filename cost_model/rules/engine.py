...
try:
    ...
    from .validators import EligibilityRule, AutoEnrollmentRule, AutoIncreaseRule, ContributionsRule, MatchRule, NonElectiveRule
    ...
    class DummyRuleModule:
        ...
        def apply(self, *args, **kwargs):
             raise NotImplementedError(f"Rule module '{self.name}' apply function called, but module failed to import: {self.error}")
    ...
    class EligibilityRule: pass
    class AutoEnrollmentRule: pass
    class AutoIncreaseRule: pass
    class ContributionsRule: pass
    class MatchRule: pass
    class NonElectiveRule: pass
    class IRSYearLimits: pass

# Import common elements from utils (absolute path from package root)
from cost_model.utils.columns import STATUS_COL, EMP_TERM_DATE, EMP_DEFERRAL_RATE, IS_ELIGIBLE, ELIGIBILITY_ENTRY_DATE
...
def set_status_based_on_termination(df, sim_year, log=None):
    ...
    log.debug(f"Status set for year {sim_year}: Active={(active_mask).sum()}, Inactive={(~active_mask).sum()}")
    return df
...
def _normalize_bool_flag(df, col, is_date=False):
    ...
    if is_date:
        ...
        df[col] = df[col].map({True: True, False: False, 'true': True, 'false': False, '1': True, '0': False, 1: True, 0: False, '': False, np.nan: False})
        ...
        try:
            ...
            logger.warning(f"Could not robustly convert column '{col}' to boolean, defaulting unhandled values to False.")
...
def apply_rules_for_year(
    ...
    log.info(f"--- Applying Plan Rules for Simulation Year {sim_year} --- RML: {RULE_MODULES_LOADED}")

    if population_df.empty:
        log.warning(f"Input DataFrame for year {sim_year} rules application is empty. Returning empty.")
    ...
    if not plan_rules_config:
        log.warning(f"No 'plan_rules' found in year_config for {sim_year}. Skipping all rule applications.")
        return df
    ...
    if not irs_limits_all_years or not isinstance(irs_limits_all_years, dict):
        log.warning(f"IRS limits missing or not a dictionary in plan_rules_config for {sim_year}. Contribution calculations might fail.")
    ...
    log.debug("Normalizing flags: is_participating, ae_opted_out, ai_opted_out, is_eligible, eligibility_entry_date")
