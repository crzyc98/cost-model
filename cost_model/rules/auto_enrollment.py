...
def apply(
    ...
    newly_eligible = (df[ELIGIBILITY_ENTRY_DATE] >= simulation_year_start_date) & (
    ...
    if ae_rules.re_enroll_existing:
        ...
        if mask.any():
            ...
            logger.info(
                f"Re-enrolled {mask.sum()} existing participants at default rate {ae_default_rate:.2%}"
            )

    # AE window closure
    within_window = (df[AE_WINDOW_END] >= simulation_year_start_date) & (
