...
def apply(df, current_scenario_config, baseline_scenario_config, simulation_year, start_year):
    ...
    if not baseline_scenario_config:
        logger.error(
            "Baseline scenario config not provided. Cannot apply plan change response logic."
        )
        return df
    ...
    is_more_generous = (
        (curr_cap > base_cap)
        or (curr_cap == base_cap and curr_rate > base_rate)
