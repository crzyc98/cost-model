...
def update(
    ...
    logger.info(
        f"Processing {events_count} events on snapshot with {initial_employee_count} employees"
    )

    try:
        # Process all events using the registry
        processing_result = processor_registry.process_all_events(
            snapshot=prev_snapshot, events=new_events, snapshot_year=snapshot_year
        )
