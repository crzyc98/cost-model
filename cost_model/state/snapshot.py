...
try:
    ...
    EVENT_COLS = ["event_id", "event_time", "employee_id", "event_type", "value_num", "value_json", "meta"]
...
def _extract_hire_details(hire_events: pd.DataFrame) -> pd.DataFrame:
    ...
    if hire_events.empty:
        return pd.DataFrame(columns=['role', 'birth_date'], index=pd.Index([], name=EMP_ID))
    ...
    for index, event in hire_events.iterrows():
        ...
        if pd.notna(event['value_json']):
            try:
                ...
                logger.warning(f"Could not parse value_json for hire event {event.get('event_id', 'N/A')} for emp {employee_id}. Error: {e}")
        # else: # Optionally check 'meta' as a fallback if needed
        #     pass

        if role is None: logger.debug(f"Could not extract 'role' for emp {employee_id} from hire event.")
        if pd.isna(birth_date): logger.debug(f"Could not extract valid 'birth_date' for emp {employee_id} from hire event.")
    ...
    details_df['birth_date'] = pd.to_datetime(details_df['birth_date']) # Already datetime, but ensures consistency
    return details_df
...
def build_full(events: pd.DataFrame) -> pd.DataFrame:
    ...
    hires = events[events['event_type'] == EVT_HIRE].drop_duplicates(subset=EMP_ID, keep='first')
    if hires.empty:
    ...
    comps = events[events['event_type'] == EVT_COMP].drop_duplicates(subset=EMP_ID, keep='last')
    last_comp = comps.set_index(EMP_ID)['value_num'].rename('current_comp') # Assumes comp is in value_num

    # 3. Get Last Termination event for each employee
    terms = events[events['event_type'] == EVT_TERM].drop_duplicates(subset=EMP_ID, keep='last')
    ...
    for col, dtype in SNAPSHOT_DTYPES.items():
        if col not in snapshot_df.columns:
            logger.warning(f"Column '{col}' missing after build_full merge, adding as NA.")
...
def update(prev_snapshot: pd.DataFrame, new_events: pd.DataFrame) -> pd.DataFrame:
    ...
    if prev_snapshot.index.name != EMP_ID:
        logger.warning(f"Previous snapshot index name is '{prev_snapshot.index.name}', expected '{EMP_ID}'. Attempting to proceed.")
        # Optionally rename: prev_snapshot.index.name = EMP_ID

    logger.info(f"Updating snapshot ({prev_snapshot.shape}) with {len(new_events)} new events...")
    ...
    first_hire_events_new = new_events[new_events['event_type'] == EVT_HIRE].drop_duplicates(subset=EMP_ID, keep='first')
    # Identify IDs present in these hire events but NOT in the previous snapshot's index
    new_hire_ids = first_hire_events_new[~first_hire_events_new[EMP_ID].isin(current_snapshot.index)][EMP_ID].unique()
    ...
    if len(new_hire_ids) > 0:
        ...
        first_hire_events_for_new_filtered = events_for_new_hires[events_for_new_hires['event_type'] == EVT_HIRE].drop_duplicates(subset=EMP_ID, keep='first')
        new_hire_details = _extract_hire_details(first_hire_events_for_new_filtered) # Indexed by EMP_ID

        # Get last comp and term events *within the new_events batch* for the new hires
        last_comp_for_new = events_for_new_hires[events_for_new_hires['event_type'] == EVT_COMP].drop_duplicates(subset=EMP_ID, keep='last').set_index(EMP_ID)['value_num']
        last_term_for_new = events_for_new_hires[events_for_new_hires['event_type'] == EVT_TERM].drop_duplicates(subset=EMP_ID, keep='last').set_index(EMP_ID)['event_time']
        ...
        new_hire_base = new_hire_base.merge(first_hire_events_for_new_filtered.set_index(EMP_ID)['event_time'].rename('hire_date'), left_index=True, right_index=True, how='left')
        new_hire_base = new_hire_base.merge(new_hire_details[['role', 'birth_date']], left_index=True, right_index=True, how='left') # Details already indexed
        new_hire_base = new_hire_base.merge(last_comp_for_new.rename('current_comp'), left_index=True, right_index=True, how='left')
        new_hire_base = new_hire_base.merge(last_term_for_new.rename('term_date'), left_index=True, right_index=True, how='left')
        ...
        for col, dtype in SNAPSHOT_DTYPES.items():
            if col not in new_hire_base.columns:
                if pd.api.types.is_datetime64_any_dtype(dtype): new_hire_base[col] = pd.NaT
                elif dtype == pd.BooleanDtype(): new_hire_base[col] = pd.NA
                elif pd.api.types.is_numeric_dtype(dtype): new_hire_base[col] = pd.NA # Use pd.NA for Float64Dtype etc.
                else: new_hire_base[col] = pd.NA # Use pd.NA for StringDtype
        ...
        current_snapshot = pd.concat([current_snapshot, new_hire_rows_df], verify_integrity=True, copy=False)
    ...
    if len(existing_ids_in_batch) > 0:
        logger.debug(f"Processing updates for {len(existing_ids_in_batch)} potentially existing employees found in new events.")
        events_for_updates = new_events[new_events[EMP_ID].isin(existing_ids_in_batch)]
        ...
        if not comp_updates.empty:
            ...
            last_comp_updates = comp_updates.sort_values('event_time').groupby(EMP_ID).tail(1)
            ...
            valid_updates = comp_update_map[comp_update_map.index.isin(active_employees)]
            if not valid_updates.empty:
                current_snapshot.loc[valid_updates.index, 'current_comp'] = valid_updates
                logger.debug(f"Applied {len(valid_updates)} compensation updates to active employees.")
            else:
                logger.debug("No compensation updates applied - all affected employees were terminated.")

        # Process termination updates
        term_updates = new_events[new_events['event_type'] == EVT_TERM]
        if not term_updates.empty:
            # Sort by event time and get the last update for each employee
            last_term_updates = term_updates.sort_values('event_time').groupby(EMP_ID).tail(1)
            ...
            logger.debug(f"Applied {len(term_update_map)} termination updates to existing employees.")
