...
from cost_model.state.schema import EMP_GROSS_COMP, EMP_ID, EVT_COLA, EVT_COMP, EVT_RAISE
        ...
        try:
            ...
            for _, comp_event in comp_events.iterrows():
                ...
                if new_compensation is None:
                    result.add_warning(
                        f"Could not extract compensation value for employee {emp_id}"
                    )
                    continue
                ...
                if EMP_ID in updated_snapshot.columns:
                    ...
                    if employee_mask.any():
                        ...
                        self.logger.debug(
                            f"Updated compensation for employee {emp_id}: {new_compensation}"
                        )
                    else:
