...
class BaseEventProcessor(ABC):
    ...
    def log_processing_start(self, events_count: int, snapshot_size: int) -> None:
        ...
        self.logger.info(f"Processing {events_count} {self.__class__.__name__} events on snapshot of {snapshot_size} employees")
    
    def log_processing_end(self, result: EventProcessorResult) -> None:
        """Log the end of event processing."""
        if result.success:
            self.logger.info(f"Successfully processed events. Affected {len(result.employees_affected)} employees")
            if result.warnings:
            ...
            if lost_employees:
                ...
                if not events.empty and EMP_ID in events.columns:
                    ...
                    if unexpected_losses:
                        issues.append(f"Unexpected employee loss: {len(unexpected_losses)} employees")
                else:
