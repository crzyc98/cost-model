...
try:
    ...
    from ..utils.columns import EMP_DEFERRAL_RATE, EMP_ID
except ImportError:
...
def run(
    snapshot: pd.DataFrame, events: pd.DataFrame, as_of: pd.Timestamp, cfg: ProactiveDecreaseConfig
) -> List[pd.DataFrame]:
    """
    ...
    if not cfg or not getattr(
        ...
        print(
            "[PD_RUN_DEBUG] Proactive Decrease rule skipped, config missing or disabled."
        )  # Using print to match existing style
        return []
    ...
    print(
        f"[PD_RUN_DEBUG] Running proactive decrease for as_of: {as_of.date()}"
    )  # Changed to print
    ...
    print(
        "[PD_RUN_DEBUG] event_time min/max:", events["event_time"].min(), events["event_time"].max()
    )
    print("[PD_RUN_DEBUG] window_start:", window_start, "as_of:", as_of)
    print(
        "[PD_RUN_DEBUG] All event_time values:\n", events[[EMP_ID, "event_time", "event_type"]]
    ...
    if snapshot.index.name != EMP_ID:
        if EMP_ID in snapshot.columns:
            ...
            print(
                f"[PD_RUN_ERROR] Snapshot missing '{EMP_ID}' column for index."
            )  # Changed to print
            return []

    print(
        f"[PD_RUN_DEBUG] Lookback window: {window_start.date()} to {as_of.date()}"
    )  # Changed to print
    ...
    if not required_event_cols.issubset(events.columns):
        print(
            f"[PD_RUN_ERROR] Events DataFrame missing one of required columns: {required_event_cols}"
        )  # Changed to print
        return []
    ...
    print(
        "[PD_RUN_DEBUG] Raw event_time[0]:",
        repr(events["event_time"].iloc[0]) if len(events) > 0 else "No events",
    )
    ...
    print(
        "[PD_RUN_DEBUG] Type check:",
        events["event_type"].isin(valid_contrib_event_types).sum(),
        "events match valid types",
    )
    print(
        "[PD_RUN_DEBUG] Start date check:",
        (event_dates >= window_start_date).sum(),
        "events >= window_start_date",
    )
    print("[PD_RUN_DEBUG] End date check:", (event_dates < as_of_date).sum(), "events < as_of_date")
    print(
        f"[PD_RUN_DEBUG] Found {len(hist)} historical contribution-related events in window."
    )  # Changed to print
    if not hist.empty:
        print(
            f"[PD_RUN_DEBUG] Sample of hist (historical events for peak calc):\n{hist.head().to_string()}"
        )
    ...
    if "value_num" in hist.columns:
        ...
        print(
            "[PD_RUN_WARNING] 'value_num' column not found in hist. Creating with NaN."
        )  # Changed to print
    ...
    print(
        f"[PD_RUN_DEBUG] Calculated peaks (sample):\n{peaks.head().to_string()}"
    )  # Changed to print
    if "C" in peaks.index:
    ...
    for emp_id_val in current_rates.index:
        ...
        if pd.isna(current_rate_val):
            print(
                f"[PD_RUN_DEBUG] emp={emp_id_val}, current_rate is NA. Skipping."
            )  # Changed to print
            continue
        ...
        print(
            f"[PD_RUN_DEBUG] emp={emp_id_val}, current_rate={current_rate:.4f}, raw_peak_from_series={peak_from_series}, effective_peak_rate={peak_rate:.4f}, threshold={cfg.threshold_pct:.4f}"
        )  # Changed to print

        if (peak_rate - current_rate) >= cfg.threshold_pct:
            delta = current_rate - peak_rate
            print(
                f"[PD_RUN_INFO] FIRED for emp={emp_id_val}. Peak: {peak_rate:.2%}, Current: {current_rate:.2%}, Delta: {delta:.2%}"
            )  # Changed to print
            output_event_rows.append(
                {
                    ...
                    "value_json": json.dumps(
                        {"old_rate": peak_rate, "new_rate": current_rate, "delta": delta}
                    ),
                    "meta": None,
                }
            )
