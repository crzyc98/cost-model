...
"""
Contributions module for generating contribution events based on plan rules.
...
try:
    ...
    EVENT_PANDAS_DTYPES = {  # Example dtypes
        ...
        "value_num": pd.Float64Dtype(),
        "value_json": pd.StringDtype(),
        "meta": pd.StringDtype(),
    }
...
def run(
    ...
    if not tiers:
        logger.info(
            "No employer match tiers found in config. Only EE contributions calculated (if any)."
        )
    ...
    try:
        ...
        if already_contributed_ids:
            logger.debug(
                f"Found {len(already_contributed_ids)} employees with existing contribution event on {as_of.date()}."
            )
    except KeyError as e:
    ...
    for emp_id in candidate_ids:
        ...
        try:
            ...
            logger.warning(
                f"Data lookup/type error for comp/rate for employee '{emp_id}'. Skipping."
            )
            continue
        ...
        contrib_payload = {
            "employee_deferral": round(ee, 2),
            "employer_match": round(er, 2),
        }
