...
try:
    ...
    from .sampling.salary import SalarySampler, DefaultSalarySampler # If needed for salary sampling
except ImportError:
    print("Warning (termination.py): Could not import sampling helpers.")
    # Define dummy functions/classes if needed
    def sample_terminations(df, *args, **kwargs): return df # Passthrough
    class DefaultSalarySampler:
         def sample_terminations(self, *args, **kwargs): return None # Return None if sampling fails
...
try:
    ...
    EMP_TERM_DATE, EMP_HIRE_DATE, EMP_GROSS_COMP = 'employee_termination_date', 'employee_hire_date', 'employee_gross_compensation'
...
def apply_turnover(
    ...
    if sampler and prev_term_salaries is not None and not prev_term_salaries.empty:
        ...
        if n_term_in_period > 0:
            ...
            try:
                draws = sampler.sample_terminations(prev_term_salaries, size=n_term_in_period, rng=rng)
                # Assign sampled compensation
                # Ensure index alignment if draws is Series
                df_with_terms.loc[term_mask, EMP_GROSS_COMP] = draws if isinstance(draws, np.ndarray) else draws.values
            except Exception as e:
                local_log.exception("Error during termination compensation sampling.")
        else:
            local_log.debug("No terminations within the specified period to sample compensation for.")
    elif sampler and (prev_term_salaries is None or prev_term_salaries.empty):
         local_log.warning("Termination sampler provided, but prev_term_salaries is missing or empty. Cannot sample termination compensation.")
