...
def build_dynamic_hazard_table(
    ...
    """
    logger.info(
        f"Building dynamic hazard table for {len(simulation_years)} years, "
...
def _validate_global_params(global_params: GlobalParameters) -> None:
    ...
    if missing_configs:
        raise ValueError(
            f"Missing required hazard configurations in global_params: {missing_configs}"
        )
...
def _calculate_termination_rates(df: pd.DataFrame, global_params: GlobalParameters) -> pd.DataFrame:
    ...
    logger.debug(
        f"Using level_discount_factor: {level_discount_factor}, min_multiplier: {min_level_discount_multiplier}"
    )
    ...
    if missing_tenure_mask.any():
        ...
        logger.warning(
            f"Missing tenure multipliers for bands: {missing_bands}. Using 1.0 as default."
        )
    ...
    level_discount_effect = np.maximum(
        min_level_discount_multiplier, 1 - level_discount_factor * (df[EMP_LEVEL] - 1)
    )
    ...
    logger.debug(
        f"Calculated termination rates - range: [{df[TERM_RATE].min():.4f}, {df[TERM_RATE].max():.4f}]"
    )
    return df


def _calculate_new_hire_termination_rates(
    df: pd.DataFrame, global_params: GlobalParameters
) -> pd.DataFrame:
    """
    ...
    logger.debug(
        f"Global params attributes: {[attr for attr in dir(global_params) if not attr.startswith('_')]}"
    )
    ...
    if hasattr(global_params, "attrition"):
        ...
        if hasattr(global_params.attrition, "new_hire_termination_rate"):
            logger.debug(
                f"Found attrition.new_hire_termination_rate: {global_params.attrition.new_hire_termination_rate}"
            )
    else:
    ...
    if hasattr(global_params, "new_hire_termination_rate"):
        ...
        logger.debug(
            f"Root level new_hire_termination_rate: {root_value} (type: {type(root_value)})"
        )
...
def _calculate_promotion_rates(df: pd.DataFrame, global_params: GlobalParameters) -> pd.DataFrame:
    ...
    if missing_tenure_mask.any():
        missing_bands = df.loc[missing_tenure_mask, EMP_TENURE_BAND].unique()
        logger.warning(
            f"Missing promotion tenure multipliers for bands: {missing_bands}. Using 1.0 as default."
        )
    ...
    level_dampener_effect = np.maximum(
        0.0, 1 - level_dampener_factor * (df[EMP_LEVEL] - 1)  # Promotion rate can be dampened to 0
    )
    ...
    logger.debug(
        f"Calculated promotion rates - range: [{df[PROMOTION_RATE].min():.4f}, {df[PROMOTION_RATE].max():.4f}]"
    )
    return df
...
def _calculate_merit_raise_rates(df: pd.DataFrame, global_params: GlobalParameters) -> pd.DataFrame:
    ...
    logger.debug(
        f"Using tenure bump bands: {merit_tenure_bump_bands}, value: {merit_tenure_bump_value}"
    )
    logger.debug(
        f"Using low level cutoff: {merit_low_level_cutoff}, bump: {merit_low_level_bump_value}"
    )
    ...
    logger.debug(
        f"Calculated merit raise rates - range: [{df[MERIT_RAISE_PCT].min():.4f}, {df[MERIT_RAISE_PCT].max():.4f}]"
    )
    return df


def _calculate_promotion_raise_rates(
    df: pd.DataFrame, global_params: GlobalParameters
) -> pd.DataFrame:
    """
...
def _calculate_cola_rates(df: pd.DataFrame, global_params: GlobalParameters) -> pd.DataFrame:
    ...
    if missing_cola_mask.any():
        ...
        logger.warning(
            f"Missing COLA rates for years: {missing_years}. Using default {default_cola}"
        )
        df[COLA_PCT] = df[COLA_PCT].fillna(default_cola)

    logger.debug(
        f"Calculated COLA rates - range: [{df[COLA_PCT].min():.4f}, {df[COLA_PCT].max():.4f}]"
    )
    return df
