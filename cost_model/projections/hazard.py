...
from cost_model.state.schema import COMP_RAISE_PCT  # Keep for backward compatibility
...
def build_hazard_table(
    years: List[int], initial_snapshot: pd.DataFrame, global_params, plan_rules_config
) -> pd.DataFrame:
    ...
    if EMP_LEVEL in initial_snapshot.columns and EMP_TENURE_BAND in initial_snapshot.columns:
        unique_levels_tenures = (
            initial_snapshot[[EMP_LEVEL, EMP_TENURE_BAND]].drop_duplicates().to_dict("records")
        )
    else:
        logger.warning(
            f"'{EMP_LEVEL}' or '{EMP_TENURE_BAND}' not in initial snapshot. Using default '1'/'all'."
        )
    ...
    if hasattr(global_params, "annual_termination_rate"):
        ...
        logger.warning(
            "global_params missing 'annual_termination_rate'. Using default 0.10. Available attributes: %s",
            dir(global_params),
        )
    ...
    if hasattr(global_params, "annual_compensation_increase_rate"):
        ...
        logger.warning(
            "global_params missing 'annual_compensation_increase_rate'. Using default 0.03. Available attributes: %s",
            dir(global_params),
        )
        global_comp_raise_pct = 0.03
    # Look for new_hire_termination_rate in attrition section first, then root level, then use default
    if hasattr(global_params, "attrition") and hasattr(
        global_params.attrition, "new_hire_termination_rate"
    ):
        ...
        logger.warning(
            "global_params missing 'new_hire_termination_rate' in both root and attrition. Using default 0.25. Available attributes: %s",
            dir(global_params),
        )
    ...
    unique_levels_tenures = [
        {EMP_LEVEL: level, EMP_TENURE_BAND: tenure_band} for level, tenure_band in all_combinations
    ]

    logger.info(
        f"Generated hazard table with {len(unique_levels_tenures)} unique (level, tenure_band) combinations"
    )
            ...
            records.append(
                {
                    ...
                    COLA_PCT: _get_cola_rate_for_year(
                        global_params, year
                    ),  # CRITICAL FIX: Look up year-specific COLA rate
    ...
    if records:
        ...
        if final_rows < initial_rows:
            ...
            logger.warning(
                f"Removed {removed_count} duplicate (year, level, tenure_band) combinations during hazard table generation"
            )
...
def load_and_expand_hazard_table(path: str = "data/hazard_table.parquet") -> pd.DataFrame:
    ...
    logger.info(
        f"DEBUG CONSTANTS: SIMULATION_YEAR='{SIMULATION_YEAR}', EMP_LEVEL='{EMP_LEVEL}', EMP_TENURE_BAND='{EMP_TENURE_BAND}'"
    )
    logger.info(
        f"DEBUG CONSTANTS: TERM_RATE='{TERM_RATE}', MERIT_RAISE_PCT='{MERIT_RAISE_PCT}', NEW_HIRE_TERMINATION_RATE='{NEW_HIRE_TERMINATION_RATE}', COLA_PCT='{COLA_PCT}'"
    )
    ...
    if actual_rename_map:
        ...
        if "tenure_band" in actual_rename_map:  # Check if 'tenure_band' was intended for renaming
            if EMP_TENURE_BAND in df.columns and "tenure_band" not in df.columns:
                logger.info(
                    f"Verification: Rename of 'tenure_band' to '{EMP_TENURE_BAND}' appears successful."
                )
            elif EMP_TENURE_BAND in df.columns and "tenure_band" in df.columns:
                logger.warning(
                    f"Verification: Rename of 'tenure_band' to '{EMP_TENURE_BAND}' PARTIALLY FAILED. Both '{EMP_TENURE_BAND}' and 'tenure_band' exist. Check constant value of EMP_TENURE_BAND."
                )
            elif EMP_TENURE_BAND not in df.columns and "tenure_band" in df.columns:
                logger.warning(
                    f"Verification: Rename of 'tenure_band' to '{EMP_TENURE_BAND}' FAILED. Original 'tenure_band' still present, '{EMP_TENURE_BAND}' is not. Check constant value of EMP_TENURE_BAND or rename_map."
                )
            else:  # EMP_TENURE_BAND not in df.columns and 'tenure_band' not in df.columns
                logger.warning(
                    f"Verification: Both 'tenure_band' and '{EMP_TENURE_BAND}' are MISSING after rename attempt. This is unexpected."
                )

        if (
            "new_hire_termination_rate" in actual_rename_map
        ):  # Check if 'new_hire_termination_rate' was intended for renaming
            if (
                NEW_HIRE_TERMINATION_RATE in df.columns
                and "new_hire_termination_rate" not in df.columns
            ):
                logger.info(
                    f"Verification: Rename of 'new_hire_termination_rate' to '{NEW_HIRE_TERMINATION_RATE}' appears successful."
                )
            elif (
                NEW_HIRE_TERMINATION_RATE in df.columns
                and "new_hire_termination_rate" in df.columns
            ):
                logger.warning(
                    f"Verification: Rename of 'new_hire_termination_rate' to '{NEW_HIRE_TERMINATION_RATE}' PARTIALLY FAILED. Both '{NEW_HIRE_TERMINATION_RATE}' and 'new_hire_termination_rate' exist. Check constant value of NEW_HIRE_TERMINATION_RATE."
                )
            elif (
                NEW_HIRE_TERMINATION_RATE not in df.columns
                and "new_hire_termination_rate" in df.columns
            ):
                logger.warning(
                    f"Verification: Rename of 'new_hire_termination_rate' to '{NEW_HIRE_TERMINATION_RATE}' FAILED. Original 'new_hire_termination_rate' still present, '{NEW_HIRE_TERMINATION_RATE}' is not. Check constant value of NEW_HIRE_TERMINATION_RATE or rename_map."
                )
            else:  # NEW_HIRE_TERMINATION_RATE not in df.columns and 'new_hire_termination_rate' not in df.columns
                logger.warning(
                    f"Verification: Both 'new_hire_termination_rate' and '{NEW_HIRE_TERMINATION_RATE}' are MISSING after rename attempt. This is unexpected."
                )
    else:
        logger.info(
            "No columns to rename: actual_rename_map is empty or no keys matched Parquet columns."
        )
    ...
    if has_old_comp_schema:
        ...
        logger.error(
            f"Hazard table missing compensation columns. Expected either 'comp_raise_pct' or 'merit_raise_pct'. Available columns: {df.columns.tolist()}"
        )
        return pd.DataFrame()
    ...
    if missing_cols:
        logger.error(
            f"Hazard table missing required columns AFTER RENAME AND VERIFICATION: {missing_cols}. Available columns: {df.columns.tolist()}"
        )
        return pd.DataFrame()
    else:
    ...
    for col in rate_columns:
        if col in result.columns:
            ...
            logger.warning(
                f"Rate column '{col}' not found in hazard table. It might be optional or missing."
            )
    ...
    if (
        EMP_LEVEL in result.columns
        and EMP_TENURE_BAND in result.columns
        and SIMULATION_YEAR in result.columns
    ):
        ...
        for year in result[SIMULATION_YEAR].unique():
            ...
            if len(year_unique) < len(year_data):
                ...
                logger.debug(
                    f"Found {duplicates_count} duplicate (level, tenure_band) combinations in hazard table for year {year}"
                )
        ...
        if final_rows < initial_rows:
            removed_count = initial_rows - final_rows
            logger.info(
                f"Removed {removed_count} duplicate (year, level, tenure_band) combinations from hazard table"
            )
        ...
        logger.info(
            f"Processed hazard table has {len(unique_combos)} unique (employee_level, tenure_band) combinations"
        )
