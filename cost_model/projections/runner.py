...
from cost_model.state.event_log import EMP_ID as EVENT_EMP_ID
...
def run_projection_engine(
    ...
    logger.info(
        f"[RUNNER] Initial snapshot: {len(current_snapshot)} total rows, {current_snapshot[EMP_ACTIVE].sum()} active rows"
    )
    ...
    if EMP_ACTIVE not in current_snapshot.columns:
        logger.error(
            "CRITICAL: Initial snapshot from create_initial_snapshot is missing the 'active' column. This should not happen. Falling back to all active."
        )
        current_snapshot[EMP_ACTIVE] = pd.Series(
            [True] * len(current_snapshot), index=current_snapshot.index
        )
    elif not pd.api.types.is_bool_dtype(current_snapshot[EMP_ACTIVE].dtype):
        logger.error(
            f"CRITICAL: Initial snapshot 'active' column (type: {current_snapshot[EMP_ACTIVE].dtype}) is not boolean. Attempting conversion or falling back."
        )
        try:
            ...
            logger.error(
                f"Failed to convert 'active' column to bool: {e_conv}. Falling back to all active."
            )
            current_snapshot[EMP_ACTIVE] = pd.Series(
                [True] * len(current_snapshot), index=current_snapshot.index
            )
    ...
    logger.info(
        f"Runner: Initial active headcount from snapshot's 'active' column: {active_count_for_runner}"
    )
    ...
    if EMP_TERM_DATE in current_snapshot.columns:
        ...
        if active_via_term_date_count != active_count_for_runner:
            logger.warning(
                f"Initial Snapshot: Mismatch between active count from '{EMP_TERM_DATE}'.isna() ({active_via_term_date_count}) "
                f"and 'active' column ({active_count_for_runner}). Proceeding with 'active' column as definitive."
            )
    else:
        logger.warning(
            f"Initial Snapshot: '{EMP_TERM_DATE}' column not present. Cannot verify 'active' column consistency."
        )
    ...
    census_template_path = getattr(global_params, "census_template_path", None) or getattr(
        config_ns, "census", None
    )
    if not census_template_path:
        logger.warning(
            "'census_template_path' not provided in global_parameters or --census flag. Proceeding with None. If hiring logic is invoked, this may cause an error."
        )
    ...
    if hasattr(plan_rules_config, "enrollment"):
        ee_contrib_event_types.append(
            getattr(plan_rules_config.enrollment, "event_type_contribution_auto", None)
        )
        ee_contrib_event_types.append(
            getattr(plan_rules_config.enrollment, "event_type_contribution_manual", None)
        )
    if hasattr(plan_rules_config, "contribution_increase"):
        ee_contrib_event_types.append(
            getattr(
                plan_rules_config.contribution_increase, "event_type_contribution_increase", None
            )
        )
    ee_contrib_event_types.append("EVT_CONTRIB_INCR")
    ee_contrib_event_types = [
        et for et in ee_contrib_event_types if et is not None and isinstance(et, str)
    ]
    ...
    if initial_snapshot_df is not None and EMP_ACTIVE in initial_snapshot_df.columns:
        ...
        logger.warning(
            "[RUNNER] Initial snapshot is missing or has no 'active' column. First year growth rate might be off."
        )
    ...
    for yr_idx, current_sim_year in enumerate(projection_sim_years):
        ...
        logger.debug(
            f"SOY {current_sim_year} - Snapshot shape: {current_snapshot.shape}, Active: {current_snapshot[EMP_ACTIVE].sum() if EMP_ACTIVE in current_snapshot else 'N/A'}"
        )
        logger.debug(
            f"SOY {current_sim_year} - Cumulative Event Log shape: {current_cumulative_event_log.shape}"
        )
        ...
        hazard_table = build_hazard_table(
            [current_sim_year], current_snapshot, global_params, plan_rules_config
        )
        ...
        if EMP_LEVEL in current_snapshot.columns and EMP_TENURE_BAND in current_snapshot.columns:
            snapshot_combos = set(
                tuple(x)
                for x in current_snapshot[[EMP_LEVEL, EMP_TENURE_BAND]].drop_duplicates().values
            )
            hazard_combos = set(
                tuple(x)
                for x in hazard_table[[EMP_LEVEL, EMP_TENURE_BAND]].drop_duplicates().values
            )
            missing_combos = snapshot_combos - hazard_combos
            if missing_combos:
                logger.warning(
                    f"[HAZARD TABLE] Year {current_sim_year}: Missing hazard table entries for combinations: {missing_combos}"
                )
        else:
            logger.warning(
                f"[HAZARD TABLE] Year {current_sim_year}: EMP_LEVEL or tenure_band not found in current snapshot; cannot check hazard table coverage."
            )
        ...
        if event_log_for_year is None or not isinstance(event_log_for_year, pd.DataFrame):
            logger.error(
                f"[RUNNER YR={current_sim_year}] run_one_year returned invalid event_log_for_year (type: {type(event_log_for_year)}). Skipping snapshot update for this year."
            )
            ...
            logger.info(
                f"[RUNNER YR={current_sim_year}] Received {len(event_log_for_year)} events from run_one_year."
            )
            ...
            current_snapshot, year_end_employee_ids = update_snapshot_with_events(
                ...
                event_log_for_year,
                pd.Timestamp(
                    f"{current_sim_year}-12-31"
                ),  # Apply events as of EOY for next snapshot
                EVENT_PRIORITY,
            )
            logger.info(
                f"[RUNNER YR={current_sim_year}] Snapshot updated. Active after events: {current_snapshot[EMP_ACTIVE].sum()}"
            )
        ...
        if event_log_for_year is not None and not event_log_for_year.empty:
            ...
            event_log_for_year[SIMULATION_YEAR] = (
                event_log_for_year[SIMULATION_YEAR].fillna(current_sim_year).astype(int)
            )

            current_cumulative_event_log = pd.concat(
                [current_cumulative_event_log, event_log_for_year], ignore_index=True
            )
            logger.info(
                f"[RUNNER YR={current_sim_year}] Cumulative event log now has {len(current_cumulative_event_log)} events."
            )
        else:
            logger.info(
                f"[RUNNER YR={current_sim_year}] No events from run_one_year to append to cumulative log."
            )
        ...
        if (
            event_log_for_year is not None
            and not event_log_for_year.empty
            and "event_type" in event_log_for_year.columns
            and "value_num" in event_log_for_year.columns
        ):
        ...
        avg_compensation_active = (
            (total_employee_gross_compensation / active_headcount_eoy)
            if active_headcount_eoy > 0
            else 0.0
        )
        ...
        if EMP_BIRTH_DATE in active_employees_df.columns and not active_employees_df.empty:
            ...
            active_employees_df.loc[:, EMP_BIRTH_DATE] = pd.to_datetime(
                active_employees_df[EMP_BIRTH_DATE], errors="coerce"
            )
            active_employees_df.loc[:, "age_years"] = (
                eoy_date_for_age_calc - active_employees_df[EMP_BIRTH_DATE]
            ).dt.days / 365.25
    ...
    if not projection_sim_years:
        ...
        logger.info(
            f"Final EOY Snapshot ({projection_sim_years[-1]}) shape: {final_eoy_snapshot.shape}"
        )
    ...
    return (
        ...
        final_eoy_snapshot,
        final_cumulative_event_log,
        summary_results_df,
        employment_status_summary_df,
    )
