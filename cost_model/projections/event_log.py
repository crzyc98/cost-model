...
from cost_model.state.event_log import (
    EMP_ID as EVT_EMP_ID,  # Alias if EMP_ID means something else here
)
from cost_model.state.event_log import (
    EVENT_COLS,
    EVENT_PANDAS_DTYPES,
...
EVT_CONTRIB_INCR = (
    "EVT_CONTRIB_INCR"  # Placeholder: define locally if not in cost_model.state.event_log
)
...
def create_initial_event_log(start_year: int) -> pd.DataFrame:
    ...
    base_event_data = [
        {
            "event_id": f"prior_evt_B_{start_year-1}",
            "event_time": pd.Timestamp(f"{start_year-1}-01-01"),
            EVT_EMP_ID: "B",
            "event_type": EVT_CONTRIB_INCR,
            "value_num": 0.05,
        },
        {
            "event_id": f"prior_evt_C_{start_year-1}",
            "event_time": pd.Timestamp(f"{start_year-1}-01-01"),
            EVT_EMP_ID: "C",
            "event_type": EVT_CONTRIB_INCR,
            "value_num": 0.10,
    ...
    for col in EVENT_COLS:
        if col not in event_log_df.columns:
            ...
            if pd.api.types.is_datetime64_any_dtype(dtype) or str(dtype).lower().startswith(
                "datetime64"
            ):
    ...
    for col, dtype_str in EVENT_PANDAS_DTYPES.items():
        if col in event_log_df.columns:
            try:
                ...
                logger.error(
                    f"Failed to cast event log column '{col}' to '{dtype_str}': {e}. Current type: {event_log_df[col].dtype}"
                )

    logger.info(
        f"Initial event log created with {len(event_log_df)} records. Dtypes:\n{event_log_df.dtypes}"
    )
    return event_log_df
