...
from cost_model.state.schema import (
    EMP_ID, EMP_HIRE_DATE, EMP_BIRTH_DATE, EMP_GROSS_COMP, EMP_ACTIVE,
    EMP_TENURE, EMP_AGE, SIMULATION_YEAR
)
...
class ValidationEngine:
    ...
    def validate_dates(self, df: pd.DataFrame) -> bool:
        ...
        if EMP_BIRTH_DATE in df.columns and EMP_HIRE_DATE in df.columns:
            ...
            if valid_rows.any():
                hire_before_birth = (df.loc[valid_rows, EMP_HIRE_DATE] < 
                                   df.loc[valid_rows, EMP_BIRTH_DATE]).sum()
                ...
                ages_at_hire = ((df.loc[valid_rows, EMP_HIRE_DATE] - 
                               df.loc[valid_rows, EMP_BIRTH_DATE]).dt.days / 365.25)
    ...
    def validate_compensation(self, df: pd.DataFrame) -> bool:
        ...
        if very_low_comp > 0:
            self.validation_warnings.append(f"Found {very_low_comp} employees with compensation < $1,000")
        
        # Check for very high compensation (> $10M)
        very_high_comp = (comp_col > 10_000_000).sum()
        if very_high_comp > 0:
            self.validation_warnings.append(f"Found {very_high_comp} employees with compensation > $10M")
    ...
    def validate_tenure_consistency(self, df: pd.DataFrame, simulation_year: int) -> bool:
        ...
        expected_tenure = ((start_date - df.loc[valid_rows, EMP_HIRE_DATE]).dt.days / 365.25).clip(lower=0)
    ...
    def validate_snapshot(self, df: pd.DataFrame, simulation_year: int, 
                         required_cols: Optional[List[str]] = None) -> Tuple[bool, List[str], List[str]]:
        """
