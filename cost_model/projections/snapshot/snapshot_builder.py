...
class SnapshotBuilder:
    ...
    def _transform_to_snapshot_format(self, census_df: pd.DataFrame, start_year: int) -> pd.DataFrame:
        """
    ...
    def build_enhanced_yearly_snapshot(
        ...
        try:
            ...
            yearly_snapshot = self._build_core_yearly_snapshot(
                start_of_year_snapshot, end_of_year_snapshot, year_events,
                employee_populations, simulation_year
            )
            ...
            yearly_snapshot = self._apply_contribution_calculations(yearly_snapshot, simulation_year)
            
            # Step 5: Calculate tenure for all employees
            yearly_snapshot = self._calculate_tenure_for_all_employees(yearly_snapshot, simulation_year)
            ...
            logger.info("Enhanced yearly snapshot for %d contains %d employees", 
                       simulation_year, len(yearly_snapshot))
            return yearly_snapshot
            
        except Exception as e:
            logger.error("Failed to build enhanced yearly snapshot for year %d: %s", 
                        simulation_year, str(e), exc_info=True)
            raise
    ...
    def _identify_employee_populations(
        ...
        """
        from cost_model.state.schema import EMP_ID, EVT_TYPE, EVT_HIRE, EVT_TERM, EMP_ACTIVE
        
        logger.debug("Identifying employee populations for year %d", simulation_year)
        
        # Get employees active at start of year
        start_of_year_active = start_of_year_snapshot[start_of_year_snapshot[EMP_ACTIVE]][EMP_ID].tolist()
        
        # Get employees hired during the year from events
        hire_events = year_events[year_events[EVT_TYPE] == EVT_HIRE] if not year_events.empty else pd.DataFrame()
        ...
        term_events = year_events[year_events[EVT_TYPE] == EVT_TERM] if not year_events.empty else pd.DataFrame()
        ...
        logger.info("Employee populations for year %d: SOY active=%d, hired=%d, terminated=%d, active during year=%d",
                   simulation_year, len(start_of_year_active), len(hired_during_year), 
                   len(terminated_during_year), len(active_during_year))
    ...
    def _build_core_yearly_snapshot(
        ...
        from cost_model.projections.snapshot_original import build_enhanced_yearly_snapshot as original_build
    ...
    def _set_employee_status_and_year(self, yearly_snapshot: pd.DataFrame, simulation_year: int) -> pd.DataFrame:
        """
    ...
    def _apply_contribution_calculations(self, yearly_snapshot: pd.DataFrame, simulation_year: int) -> pd.DataFrame:
        """
    ...
    def _calculate_tenure_for_all_employees(self, yearly_snapshot: pd.DataFrame, simulation_year: int) -> pd.DataFrame:
        """
    ...
    def _apply_age_calculations(self, yearly_snapshot: pd.DataFrame, simulation_year: int) -> pd.DataFrame:
        """
        ...
        yearly_snapshot = self.data_transformer.calculate_age_information(yearly_snapshot, simulation_year)
        
        return yearly_snapshot
    
    def _validate_and_log_snapshot_summary(self, yearly_snapshot: pd.DataFrame, simulation_year: int) -> None:
        """
        ...
        is_valid, errors, warnings = self.validation_engine.validate_snapshot(yearly_snapshot, simulation_year)
