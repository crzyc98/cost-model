...
from .logging_utils import get_snapshot_logger, timing_decorator, log_dataframe_info, progress_context
from .types import (
    FilePath, SimulationYear, CompensationAmount, ColumnName,
    CensusLoaderConfig, ValidationResult as ValidationResultType
)
...
class CensusLoader:
    ...
    def load_census_file(self, census_path: FilePath) -> pd.DataFrame:
        ...
        logger.info("Loading census data", file_path=str(census_path), file_size_mb=f"{census_path.stat().st_size / 1024 / 1024:.2f}")
    ...
    def standardize_column_names(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if rename_mapping:
            logger.debug("Standardizing column names", mapping_count=len(rename_mapping), mappings=rename_mapping)
    ...
    def filter_terminated_employees(self, df: pd.DataFrame, start_year: int) -> pd.DataFrame:
        ...
        if EMP_TERM_DATE in df.columns:
            ...
            terminated_before_start = (
                ...
                (df[EMP_TERM_DATE] < start_date)
            )
            df = df[~terminated_before_start].copy()
            
            filtered_count = initial_count - len(df)
            if filtered_count > 0:
                logger.info("Filtered terminated employees", 
        ...
        if EMP_ACTIVE in df.columns:
            ...
            if df[EMP_ACTIVE].dtype == 'object':
                ...
                df[EMP_ACTIVE] = df[EMP_ACTIVE].astype(str).str.lower().isin(['true', '1', 'yes', 'active'])
            ...
            if active_filtered > 0:
                logger.info("Filtered inactive employees", 
                          filtered_count=active_filtered,
                          remaining_count=len(df))
        
        logger.info("Employee filtering complete", 
                   final_count=len(df),
    ...
    def initialize_employee_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if EMP_GROSS_COMP not in df.columns:
            logger.warning(f"No compensation column found, using default: ${DEFAULT_COMPENSATION:,.0f}")
