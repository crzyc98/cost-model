...
from .constants import AGE_BANDS, LEVEL_BASED_DEFAULTS, TENURE_BANDS
...
class SnapshotTransformer:
    ...
    def _ensure_schema_migration(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if not self._migration_done:
            df, migration_result = migrate_legacy_columns(
                df, schema_type="snapshot", strict_mode=False
            )
    ...
    def apply_tenure_calculations(
        self, df: pd.DataFrame, reference_date: Optional[datetime] = None
    ) -> pd.DataFrame:
        """
        ...
        if hire_date_col not in df.columns:
            raise SnapshotBuildError(
                f"Cannot calculate tenure: {hire_date_col} column missing. Available columns: {list(df.columns)}"
            )
    ...
    def apply_contribution_calculations(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if comp_col not in df.columns:
            raise SnapshotBuildError(
                f"Cannot calculate contributions: {comp_col} column missing. Available columns: {list(df.columns)}"
            )
                ...
                if emp_id_col in level_result.columns and emp_id_col in original_df.columns:
                    ...
                    for col in [level_col, level_source_col]:
                        if col in level_mapping.columns:
                            original_df[col] = (
                                original_df[emp_id_col]
                                .map(level_mapping[col])
                                .fillna(original_df.get(col, pd.NA))
                            )
    ...
    def normalize_compensation_by_level(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if level_col not in df.columns:
            logger.warning(
                f"No job level column {level_col} available for compensation normalization"
            )
            return df
        ...
        if missing_count > 0:
            ...
            if still_missing_count > 0:
                ...
                logger.warning(
                    f"Used global default compensation for {still_missing_count} employees"
                )
    ...
    def _fallback_job_level_inference(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if comp_col not in df.columns:
            logger.error(
                f"No compensation column {comp_col} found. Available columns: {list(df.columns)}"
            )
            # Set default levels
            df[level_col] = "BAND_1"
