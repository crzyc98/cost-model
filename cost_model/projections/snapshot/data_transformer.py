...
class DataTransformer:
    ...
    def __init__(self):
        self.tenure_bands = {"<1": (0, 1), "1-3": (1, 3), "3-5": (3, 5), "5+": (5, float("inf"))}
    ...
    def calculate_tenure(self, census_df: pd.DataFrame, start_year: int) -> pd.DataFrame:
        ...
        logger.info(
            f"Calculated tenure for {len(census_df)} employees. "
    ...
    def set_employment_status(self, census_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if has_term_dates:
            ...
            logger.info(
                f"Setting active status based on termination dates: "
    ...
    def create_snapshot_structure(self, census_df: pd.DataFrame, start_year: int) -> pd.DataFrame:
        ...
        snapshot_data = {
            ...
            EMP_LEVEL_SOURCE: census_df.get(
        ...
        logger.info(
            f"Created snapshot with {len(snapshot_df)} employees and {len(snapshot_df.columns)} columns"
        )

        return snapshot_df

    def transform_census_to_snapshot(
