...
from .constants import REQUIRED_CENSUS_COLUMNS, SUPPORTED_FILE_FORMATS, VALIDATION_THRESHOLDS
    ...
    def _validate_data_ranges(self, df: pd.DataFrame, result: ValidationResult):
        ...
        if EMP_AGE in df.columns:
            ...
            if not age_data.empty:
                ...
                if invalid_ages > 0:
                    result.add_warning(
                        f"{invalid_ages} employees have ages outside valid range "
                        f"({min_age}-{max_age})"
                    )
        ...
        if EMP_TENURE in df.columns:
            ...
            if not tenure_data.empty:
                ...
                if invalid_tenure > 0:
                    result.add_warning(
                        f"{invalid_tenure} employees have tenure > {max_tenure} years"
                    )
        ...
        if EMP_DEFERRAL_RATE in df.columns:
            ...
            if not deferral_data.empty:
                ...
                if invalid_deferral > 0:
                    result.add_warning(
                        f"{invalid_deferral} employees have invalid deferral rates "
...
def validate_and_log_results(result: ValidationResult, operation_name: str):
    ...
    if result.warnings:
        logger.info(
            f"Validation completed for {operation_name} with {len(result.warnings)} warnings"
        )
    else:
