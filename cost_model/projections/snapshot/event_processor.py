...
from .constants import COMPENSATION_EXTRACTION_PRIORITIES, DEFAULT_COMPENSATION, LEVEL_BASED_DEFAULTS
from .exceptions import EventProcessingError, CompensationExtractionError
...
class EventProcessor:
    ...
    def update_snapshot_with_events(
        ...
        try:
            ...
            from cost_model.state.snapshot_update import update_snapshot_with_events as original_func
    ...
    def extract_compensation_for_employee(
        ...
        emp_id: str, 
        events: pd.DataFrame,
        default_compensation: Optional[float] = None
    ...
    def _extract_from_hire_events(self, emp_id: str, emp_events: pd.DataFrame) -> CompensationExtractionResult:
        """Extract compensation from hiring events."""
        hire_events = emp_events[emp_events['event_type'] == 'hire']
        
        if hire_events.empty:
            return CompensationExtractionResult(compensation=None, source='hire_events', confidence='low', details={})
    ...
    def _extract_from_promotion_events(self, emp_id: str, emp_events: pd.DataFrame) -> CompensationExtractionResult:
        ...
        if promotion_events.empty:
            return CompensationExtractionResult(compensation=None, source='promotion_events', confidence='low', details={})
    ...
    def _extract_from_compensation_events(self, emp_id: str, emp_events: pd.DataFrame) -> CompensationExtractionResult:
        ...
        if comp_events.empty:
            return CompensationExtractionResult(compensation=None, source='compensation_events', confidence='low', details={})
    ...
    def _extract_by_level_default(self, emp_id: str, emp_events: pd.DataFrame) -> CompensationExtractionResult:
        ...
        return CompensationExtractionResult(compensation=None, source='default_by_level', confidence='low', details={})