...
class StatusProcessor:
    ...
    def _determine_employee_status_eoy(self, row: pd.Series) -> str:
        ...
        if is_active and not has_term_date and not is_exited:
            ...
            logger.warning(
                f"Unclear employee status for {row.get('employee_id', 'unknown')}, defaulting to ACTIVE"
            )
            return "ACTIVE"
    ...
    def validate_employee_status(self, snapshot: pd.DataFrame) -> Dict[str, Any]:
        ...
        if null_status > 0:
            validation_results["warnings"].append(f"Found {null_status} employees with null status")
    ...
    def _validate_status_consistency(
        self, snapshot: pd.DataFrame, validation_results: Dict[str, Any]
    ) -> None:
        """
        ...
        if not active_employees.empty and term_date_col in snapshot.columns:
            active_with_term_date = active_employees[active_employees[term_date_col].notna()]
