...
class CensusProcessor:
    ...
    def load_census_file(self, census_path: Union[str, Path]) -> pd.DataFrame:
        ...
        try:
            ...
            try:
                ...
                logger.error("Failed to read census file as either Parquet or CSV: %s", 
                           str(csv_error), exc_info=True)
                raise csv_error
    ...
    def process_census_data(self, census_path: Union[str, Path], start_year: int) -> pd.DataFrame:
        ...
        """
        logger.info("Processing census data from %s for start year %d", str(census_path), start_year)
        ...
        logger.info("Loaded census data with %d records. Columns: %s", 
                   len(census_df), census_df.columns.tolist())
