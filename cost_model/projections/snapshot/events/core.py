...
class EventProcessor:
    ...
    def process_events(
        ...
        context = EventContext(
            simulation_year=simulation_year, reference_date=reference_date, config=self.config
        )

        result = EventProcessingResult(success=True, updated_snapshot=snapshot.copy())
    ...
    def validate_events_only(
        self, events: pd.DataFrame, simulation_year: int, reference_date: Optional[datetime] = None
    ) -> List[str]:
        """
    ...
    def _register_default_handlers(self):
        ...
        try:
            ...
            self.registry.register_handler(
                EventTypes.NEW_HIRE_TERMINATION, TerminationEventHandler()
            )
    ...
    def _validate_inputs(
        self, snapshot: pd.DataFrame, events: pd.DataFrame, result: EventProcessingResult
    ):
        """Validate input data."""
    ...
    def _preprocess_events(
        self, events: pd.DataFrame, context: EventContext, result: EventProcessingResult
    ) -> pd.DataFrame:
        ...
        try:
            ...
            processed_events["_processing_priority"] = (
                processed_events[EventColumns.EVENT_TYPE].map(event_priority_map).fillna(99)
            )  # Unknown events get low priority

            processed_events = processed_events.sort_values(
                [EventColumns.EVENT_DATE, "_processing_priority", EventColumns.EMP_ID]
    ...
    def _validate_final_snapshot(
        self, snapshot: pd.DataFrame, context: EventContext, result: EventProcessingResult
    ):
        ...
        try:
            ...
            validation_result = validate_snapshot_schema(
                snapshot, strict_mode=False, check_data_quality=True
            )
