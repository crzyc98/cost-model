...
class EventValidator:
    ...
    def validate_event_consistency(self, 
        ...
        try:
            ...
            term_events = events[
                events[EventColumns.EVENT_TYPE].isin([EventTypes.TERMINATION, EventTypes.NEW_HIRE_TERMINATION])
            ]
            
            if not term_events.empty and all(col in snapshot.columns for col in [SnapshotColumns.EMP_ID, SnapshotColumns.EMP_ACTIVE]):
            ...
            if field in events.columns:
                try:
                    ...
                    if invalid_numeric.any():
                        errors.append(f"Found {invalid_numeric.sum()} invalid numeric values in {field}")
                except Exception as e:
    ...
    def _validate_data_quality(self, 
        ...
        if EventColumns.DEFERRAL_RATE in events.columns:
            ...
            if invalid_rates.any():
                errors.append(f"Found {invalid_rates.sum()} events with invalid deferral rates (outside 0-1)")
    ...
    def _validate_event_specific_rules(self, 
                                      events: pd.DataFrame, 
                                      context: EventContext) -> List[str]:
    ...
    def _validate_hire_events(self, 
        ...
        if EventColumns.EVENT_DATE in hire_events.columns:
            ...
            very_old_hires = hire_dates < (context.reference_date - timedelta(days=365*50))  # 50 years ago
            very_future_hires = hire_dates > (context.reference_date + timedelta(days=365*2))  # 2 years future
            ...
            if very_future_hires.any():
                errors.append(f"Found {very_future_hires.sum()} hire events with future dates beyond reasonable range")
        
        return errors
    
    def _validate_termination_events(self, 
    ...
    def _validate_promotion_events(self, 
    ...
    def _validate_compensation_events(self, 
