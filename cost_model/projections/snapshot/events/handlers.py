...
class HireEventHandler(BaseEventProcessor):
        ...
        try:
            ...
            if validation_errors:
                raise EventProcessingError(
                    f"Hire event validation failed: {validation_errors}",
                    event_type=EventTypes.HIRE
                )
    ...
    def validate_events(self, 
        ...
        if EventColumns.GROSS_COMPENSATION in events.columns:
            ...
            if zero_comp.any():
                errors.append(f"Found {zero_comp.sum()} hire events with zero or negative compensation")
        
        return errors
    
    def _create_new_employee_records(self, 
    ...
    def _calculate_initial_contributions(self, new_employees: pd.DataFrame) -> pd.DataFrame:
        ...
        employee_contrib_eligible = new_employees[SnapshotColumns.EMP_CONTRIBUTION].clip(upper=max_match_base)
...
class TerminationEventHandler(BaseEventProcessor):
    ...
    def process_events(self, 
        ...
        try:
            ...
            self.stats.add_processed_event(success=False)
            raise EventProcessingError(f"Failed to process termination events: {str(e)}", "TERMINATION")
    ...
    def validate_events(self, 
        ...
        if EventColumns.EVENT_DATE in events.columns:
            ...
            if future_terms.any():
                ...
                self.logger.warning(f"Found {future_terms.sum()} termination events with future dates")
...
class PromotionEventHandler(BaseEventProcessor):
    ...
    def process_events(self, 
        ...
        try:
            ...
            if EventColumns.GROSS_COMPENSATION in events.columns:
                updates[SnapshotColumns.EMP_GROSS_COMP] = events[EventColumns.GROSS_COMPENSATION].tolist()
            
            # Update deferral rate if provided
            if EventColumns.DEFERRAL_RATE in events.columns:
                updates[SnapshotColumns.EMP_DEFERRAL_RATE] = events[EventColumns.DEFERRAL_RATE].tolist()
            ...
            if any(col in updates for col in [SnapshotColumns.EMP_GROSS_COMP, SnapshotColumns.EMP_DEFERRAL_RATE]):
            ...
            self.stats.add_processed_event(success=False)
            raise EventProcessingError(f"Failed to process promotion events: {str(e)}", EventTypes.PROMOTION)
    ...
    def _recalculate_contributions(self, 
                                 snapshot: pd.DataFrame,
                                 employee_mask: pd.Series) -> pd.DataFrame:
        ...
        employee_contrib_eligible = promoted_employees[SnapshotColumns.EMP_CONTRIBUTION].clip(upper=max_match_base)
        snapshot.loc[employee_mask, SnapshotColumns.EMPLOYER_MATCH_CONTRIBUTION] = employee_contrib_eligible * 0.5
...
class CompensationEventHandler(BaseEventProcessor):
    ...
    def process_events(self, 
        ...
        try:
            ...
            self.stats.add_processed_event(success=False)
            raise EventProcessingError(f"Failed to process compensation events: {str(e)}", EventTypes.COMPENSATION)
    ...
    def validate_events(self, 
        ...
        if EventColumns.GROSS_COMPENSATION not in events.columns:
            ...
            zero_comp = events[EventColumns.GROSS_COMPENSATION] <= 0
            if zero_comp.any():
                errors.append(f"Found {zero_comp.sum()} compensation events with zero or negative amount")
    ...
    def _recalculate_contributions(self, 
        ...
        employee_contrib_eligible = affected_employees[SnapshotColumns.EMP_CONTRIBUTION].clip(upper=max_match_base)
        snapshot.loc[employee_mask, SnapshotColumns.EMPLOYER_MATCH_CONTRIBUTION] = employee_contrib_eligible * 0.5
