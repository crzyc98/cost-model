...
class BaseEventProcessor(ABC):
    ...
    def _validate_common_fields(self, events: pd.DataFrame, context: EventContext) -> List[str]:
        """
    ...
    def _update_snapshot_safely(
        ...
        try:
            ...
            for column, values in updates.items():
                ...
                if isinstance(values, (list, pd.Series)):
                    ...
                    if len(values) != affected_count:
                        raise ValueError(
                            f"Mismatch between mask size ({affected_count}) and values size ({len(values)})"
                        )
            ...
            self.logger.debug(
                f"Successfully applied {operation_name} to {affected_count} employees"
            )
    ...
    def _get_employee_mask(self, snapshot: pd.DataFrame, employee_ids: List[str]) -> pd.Series:
        """
    ...
    def _log_processing_summary(
        self, event_type: str, events_count: int, processing_time: float
    ) -> None:
        """
