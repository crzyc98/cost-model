...
def plot_projection_results(summary_df: pd.DataFrame, output_dir: Path):
    ...
    if missing_cols:
        logger.warning(
            f"Summary DataFrame missing required columns for plotting: {missing_cols}. "
    ...
    try:
        ...
        ax1.plot(
            plot_df[schema.SUMMARY_YEAR],
            plot_df[schema.SUMMARY_ACTIVE_HEADCOUNT],
            color=color,
            marker="o",
            linestyle="-",
        )
        ...
        ax2.plot(
            plot_df[schema.SUMMARY_YEAR],
            plot_df[schema.SUMMARY_TERMINATIONS],
            color=color,
            marker="x",
            linestyle="--",
        )
        ...
        if contrib_col:
            ...
            ax_contrib.bar(
                plot_df[schema.SUMMARY_YEAR], plot_df[contrib_col], color=color, alpha=0.7
            )
            ax_contrib.tick_params(axis="y", labelcolor=color)
            ax_contrib.yaxis.set_major_formatter(
...
def save_detailed_results(
    ...
    if (
        "employee_termination_date" in final_snapshot.columns
        and "simulation_year" in final_snapshot.columns
    ):
        ...
        logger.info(
            f"Filtered out {len(final_snapshot) - len(filtered_snapshot)} employees terminated in prior years"
        )
    ...
    if "employee_role" in final_snapshot_clean.columns:
        ...
        final_snapshot_clean["employee_role"] = final_snapshot_clean["employee_role"].replace(
            "nan", pd.NA
        )
        # Convert to nullable string dtype
        final_snapshot_clean["employee_role"] = final_snapshot_clean["employee_role"].astype(
            pd.StringDtype()
        )
        logger.debug(
            f"Cleaned employee_role column in final snapshot: {final_snapshot_clean['employee_role'].dtype}"
        )
        ...
        if final_snapshot_clean[col].dtype == "object":
            ...
            if len(sample_values) > 0:
                ...
                if has_strings and has_numbers:
                    logger.warning(
                        f"Column '{col}' has mixed types in final snapshot, converting to string"
                    )
                    final_snapshot_clean[col] = (
                        final_snapshot_clean[col]
                        .astype(str)
                        .replace("nan", pd.NA)
    ...
    if (
        full_event_log is None
        or not isinstance(full_event_log, pd.DataFrame)
        or full_event_log.empty
    ):
        logger.warning(
            "No event log data provided or empty event log. Creating an empty event log with expected schema."
        )
    ...
    try:
        ...
        logger.info(
            f"Final cumulative event log saved to {event_log_path} with {len(full_event_log)} events"
        )
        ...
        if event_log_path.exists():
            ...
            try:
                ...
                if not sample.empty:
                    logger.info(
                        f"Sample event types: {sample['event_type'].value_counts().to_dict()}"
                    )
            except Exception as e:
    ...
    if full_event_log is not None and not full_event_log.empty:
        if "simulation_year" in full_event_log.columns:
            hires_per_year = (
                full_event_log.query("event_type == @EVT_HIRE").groupby("simulation_year").size()
            )
            terms_per_year = (
                full_event_log.query("event_type == @EVT_TERM").groupby("simulation_year").size()
            )
            ...
            if canonical_year not in summary_to_save.columns:
                ...
                # Legacy fallback: if we have Projection Year, it might have been used instead
                    ...
                    logger.warning(
                        f"No {canonical_year} column found in summary. Creating one from event log years."
                    )
                    ...
                    if len(event_years) > 0:
                        if not summary_to_save.empty and len(summary_to_save) == len(event_years):
                            ...
                            logger.warning(
                                "Cannot create year column: summary rows don't match event years."
                            )
            ...
            if canonical_year in summary_to_save.columns:
                summary_to_save["new_hires"] = (
                    summary_to_save[canonical_year].map(hires_per_year).fillna(0).astype(int)
                )
                summary_to_save["terminated_employees"] = (
                    summary_to_save[canonical_year].map(terms_per_year).fillna(0).astype(int)
                )
            else:
                logger.error(
                    f"Failed to create '{canonical_year}' column in summary_to_save. Cannot map hires/terminations."
                )
            ...
            logger.warning(
                "simulation_year column missing in event log; cannot compute new_hires/terminated_employees by year."
            )
    else:
    ...
    if employment_status_summary_df is not None and not employment_status_summary_df.empty:
        ...
        for col in employment_status_summary_df.columns:
            ...
            if col in summary_cols_seen and col.lower() not in [
                "year",
                "projection year",
                "simulation_year",
            ]:
        ...
        if emp_year_col is None:
            logger.warning(
                "No year column found in employment status. Creating index-based year column."
            )
        ...
        if summary_year_col != emp_year_col:
            ...
            if summary_year_col != standard_year_col:
                ...
                logger.info(
                    f"Created standard '{standard_year_col}' column in summary from '{summary_year_col}'"
                )
            ...
            if emp_year_col != standard_year_col:
                ...
                logger.info(
                    f"Created standard '{standard_year_col}' column in employment status from '{emp_year_col}'"
                )
        ...
        try:
            ...
            merged_df = summary_clean.merge(emp_clean, on=merge_col, how="left")

            logger.info(
                f"Successfully merged summary and employment status data with {len(merged_df)} rows"
            )
            logger.debug(
                f"Merged columns ({len(merged_df.columns)}): {sorted(merged_df.columns.tolist())}"
            )
    ...
    if yearly_snapshots:
        ...
        for year, snapshot_df in yearly_snapshots.items():
            ...
            if "employee_role" in snapshot_df_clean.columns:
                ...
                # Replace 'nan' strings with proper NA values for string dtype
                snapshot_df_clean["employee_role"] = snapshot_df_clean["employee_role"].replace(
                    "nan", pd.NA
                )
                # Convert to nullable string dtype
                snapshot_df_clean["employee_role"] = snapshot_df_clean["employee_role"].astype(
                    pd.StringDtype()
                )
                logger.debug(
                    f"Cleaned employee_role column for year {year}: {snapshot_df_clean['employee_role'].dtype}"
                )
            ...
            for col in snapshot_df_clean.columns:
                if snapshot_df_clean[col].dtype == "object":
                    ...
                    if len(sample_values) > 0:
                        ...
                        if has_strings and has_numbers:
                            logger.warning(
                                f"Column '{col}' has mixed types, converting to string for year {year}"
                            )
    ...
    logger.debug(
        "NOTE: For optimal schema compliance, upstream summary generation processes "
