...
from cost_model.state.schema import (  # Import all required column constants
    EMP_ACTIVE,
    EMP_BIRTH_DATE,
    EMP_DEFERRAL_RATE,
    EMP_EXITED,
...
def create_initial_snapshot(start_year: int, census_path: Union[str, Path]) -> pd.DataFrame:
    ...
    if term_col:
        ...
        census_df = census_df[
            census_df[term_col].isna() | (census_df[term_col] > pd.Timestamp(f"{start_year}-01-01"))
        ]
    ...
    if has_term_dates:
        ...
        logger.info(
            f"Setting active status based on termination dates: {active_status.sum()} active out of {len(census_df)}"
        )
    else:
    ...
    if EMP_TENURE in initial_data:
        # Define tenure bands
        bands = {"<1": (0, 1), "1-3": (1, 3), "3-5": (3, 5), "5+": (5, float("inf"))}
        ...
        assert (
            "tenure_band" not in snapshot_df.columns or EMP_TENURE_BAND == "tenure_band"
    ...
    if has_term_dates:
        ...
        if EMP_LEVEL in census_df.columns and not census_df[EMP_LEVEL].isna().all():
            ...
            temp_df = pd.DataFrame(
                {EMP_ID: census_df[EMP_ID], EMP_GROSS_COMP: census_df[EMP_GROSS_COMP]}
            )
            ...
            temp_df = ingest_with_imputation(
                temp_df, comp_col=EMP_GROSS_COMP, target_level_col=target_level_col
            )
            ...
            logger.info(
                f"Inferred job levels: {temp_df[target_level_col].value_counts(dropna=False).to_dict()}"
            )
            logger.debug(
                f"Job levels distribution: {temp_df[target_level_col].value_counts(dropna=False)}"
            )
    else:
        ...
        initial_data[EMP_LEVEL_SOURCE] = pd.Series(
            ["default"] * len(initial_data[EMP_ID]), dtype="string"
        )
    ...
    if SIMULATION_YEAR not in snapshot_df.columns:
        logger.debug(
            f"Adding missing {SIMULATION_YEAR} column to snapshot_df with value {start_year}"
        )
    ...
    snapshot_df[EMP_TENURE] = (
        current_date_for_tenure - snapshot_df[EMP_HIRE_DATE]
    ).dt.days / 365.25
    ...
    snapshot_df[EMP_TENURE_BAND] = pd.cut(
        snapshot_df[EMP_TENURE], bins=bins, labels=labels, right=False
    ...
    for col in SNAPSHOT_COL_NAMES:
        if col not in snapshot_df.columns:
            logger.warning(
                f"Snapshot column '{col}' defined in SNAPSHOT_COL_NAMES is missing. Will be added with default value."
            )
            ...
            if pd.api.types.is_datetime64_any_dtype(dtype):
                ...
                snapshot_df[col] = pd.Categorical(
                    [pd.NA] * len(snapshot_df), categories=dtype.categories, ordered=dtype.ordered
                )
            else:  # Default to StringDtype for others like 'object' or 'string'
                snapshot_df[col] = pd.NA
                if pd.api.types.is_string_dtype(dtype) or str(dtype) == "object":
                    snapshot_df[col] = snapshot_df[col].astype(
                        pd.StringDtype()
                    )  # Ensure it's nullable string
    ...
    if EMP_ID not in snapshot_df.columns and EMP_ID in SNAPSHOT_COL_NAMES:
        ...
        snapshot_df[EMP_ID] = snapshot_df[EMP_ID].astype(
            SNAPSHOT_DTYPES.get(EMP_ID, pd.StringDtype())
        )
    ...
    for col in SNAPSHOT_COL_NAMES:
        ...
        if col not in snapshot_df.columns:
            logger.warning(
                f"Column '{col}' not in snapshot_df but is in SNAPSHOT_COL_NAMES. Adding it with NAs."
            )
            if pd.api.types.is_datetime64_any_dtype(dtype):
                snapshot_df[col] = pd.NaT
            elif isinstance(dtype, pd.CategoricalDtype):
    ...
    for col, dtype_str in SNAPSHOT_DTYPES.items():
        if col in snapshot_df.columns:
            try:
                ...
                logger.error(
                    f"Failed to cast snapshot column '{col}' to '{dtype_str}': {e}. Current type: {snapshot_df[col].dtype}. Data sample: {snapshot_df[col].head().to_dict()}"
                )
                # Potentially skip casting or re-raise depending on strictness

    logger.info(
        f"Initial snapshot created with {len(snapshot_df)} records. Columns: {snapshot_df.columns.tolist()}"
    )
    ...
    logger.debug(
        f"Applying age and age band calculations for initial snapshot as of {as_of_date_for_age}"
    )
    ...
    if EMP_BIRTH_DATE in snapshot_df.columns:
        ...
        logger.debug(
            f"Age calculations completed for initial snapshot. Sample ages: {snapshot_df[EMP_AGE].head().tolist()}"
        )
        logger.debug(f"Sample age bands: {snapshot_df[EMP_AGE_BAND].head().tolist()}")
    else:
        logger.warning(
            f"Birth date column '{EMP_BIRTH_DATE}' not found in initial snapshot. Age calculations skipped."
        )
...
def _extract_compensation_for_employee(
    emp_id: str, year_events: pd.DataFrame, simulation_year: int, logger: logging.Logger
) -> float:
    """
    ...
    from cost_model.state.schema import EMP_ID, EMP_TERM_DATE, EVT_COMP, EVT_HIRE
    ...
    comp_events = year_events[
        (year_events["event_type"] == EVT_COMP) & (year_events[EMP_ID] == emp_id)
    ]
    ...
    if not comp_events.empty:
        ...
        if not comp_events.empty:
            ...
            if pd.notna(latest_comp_event["value_num"]) and latest_comp_event["value_num"] > 0:
                logger.info(
                    f"Using EVT_COMP compensation for employee {emp_id}: {latest_comp_event['value_num']}"
                )
    ...
    if not hire_events.empty:
        ...
        if pd.notna(hire_event.get("value_json")):
            try:
                ...
                if isinstance(payload, dict) and "compensation" in payload:
                    ...
                    if comp_value > 0:
                        logger.info(
                            f"Using hire event compensation for employee {emp_id}: {comp_value}"
                        )
                        return comp_value
            except (json.JSONDecodeError, TypeError, ValueError) as e:
                logger.warning(
                    f"Failed to parse compensation from hire event for employee {emp_id}: {e}"
                )
        ...
        logger.info(
            f"Using level-based default compensation for employee {emp_id} (level={level}): {level_comp}"
        )
        return level_comp
...
def build_enhanced_yearly_snapshot(
    ...
    if year_events is not None and not year_events.empty and "event_type" in year_events.columns:
        ...
        if not hire_events.empty and EMP_ID in hire_events.columns:
            ...
            logger.info(
                f"Found {len(hired_this_year)} employees hired during year {simulation_year}"
            )
    ...
    if year_events is not None and not year_events.empty and "event_type" in year_events.columns:
        ...
        if not term_events.empty and EMP_ID in term_events.columns:
            ...
            regular_terms = (
                len(regular_term_events[EMP_ID].unique()) if not regular_term_events.empty else 0
            )
            new_hire_term_events = year_events[year_events["event_type"] == EVT_NEW_HIRE_TERM]
            new_hire_terms = (
                len(new_hire_term_events[EMP_ID].unique()) if not new_hire_term_events.empty else 0
            )
            logger.info(
                f"Found {len(terminated_this_year)} employees terminated during year {simulation_year} "
    ...
    logger.info(
        f"Total employees active during year {simulation_year}: {len(active_during_year_ids)} "
    ...
    logger.info(
        f"Building yearly snapshot from SOY snapshot + EOY snapshot + events for {len(active_during_year_ids)} employees"
    )
    ...
    eoy_employee_ids = (
        set(eoy_employees[EMP_ID].unique()) if EMP_ID in eoy_employees.columns else set()
    )
    missing_employee_ids = active_during_year_ids - eoy_employee_ids

    if missing_employee_ids:
        logger.info(
            f"Found {len(missing_employee_ids)} employees active during year but missing from EOY snapshot (likely terminated)"
        )
        ...
        if EMP_ID in start_of_year_snapshot.columns:
            ...
            if not soy_missing_no_term.empty:
                logger.warning(
                    f"Found {len(soy_missing_no_term)} employees in SOY but missing from EOY without termination events: {soy_missing_no_term[EMP_ID].tolist()}"
                )
        ...
        new_hire_terminated_ids = missing_employee_ids.intersection(hired_this_year).intersection(
            terminated_this_year
        )
        if new_hire_terminated_ids and year_events is not None and not year_events.empty:
            logger.info(
                f"Reconstructing {len(new_hire_terminated_ids)} terminated new hires from events"
            )
                ...
                if not hire_events.empty and not term_events.empty:
                    ...
                    if pd.notna(hire_event.get("value_json")):
                        try:
                            payload = parse_hire_payload(hire_event["value_json"])
                            birth_date = (
                                pd.to_datetime(payload.get("birth_date"), errors="coerce")
                            ...
                            logger.warning(
                                f"Failed to parse hire event details for employee {emp_id}: {e}"
                            )
                    ...
                    logger.info(
                        f"Calculated tenure for terminated new hire {emp_id}: {tenure_years:.3f} years ({tenure_band}) from {hire_date.date()} to {term_date.date()}"
                    )
                    ...
                    logger.debug(
                        f"Reconstructed terminated new hire {emp_id} with compensation {compensation}"
                    )
                else:
                    logger.warning(
                        f"Could not find both hire and termination events for employee {emp_id}"
                    )
                ...
                if emp_id in terminated_this_year:
                    ...
                    if year_events is not None and not year_events.empty:
                        ...
                        if not emp_term_events.empty:
                            ...
                            missing_employees.loc[
                                missing_employees[EMP_ID] == emp_id, EMP_TERM_DATE
                            ] = term_date
                            missing_employees.loc[
                                missing_employees[EMP_ID] == emp_id, EMP_ACTIVE
                            ] = False
    ...
    if EMP_BIRTH_DATE not in yearly_snapshot.columns:
        ...
        logger.warning(
            f"Added default {EMP_BIRTH_DATE} column to yearly snapshot - age calculations may be inaccurate"
        )
    ...
    logger.info(
        f"Applied tenure calculations to all {len(yearly_snapshot)} employees in yearly snapshot"
    )
    ...
    if EMP_STATUS_EOY in yearly_snapshot.columns:
        ...
        logger.info(
            f"Year {simulation_year} snapshot status distribution: {status_counts.to_dict()}"
        )
    ...
    if "employee_gross_compensation" in yearly_snapshot.columns:
        ...
        if not terminated_employees.empty:
            ...
            logger.info(
                f"  - Compensation stats: mean={comp_stats['mean']:.2f}, min={comp_stats['min']:.2f}, max={comp_stats['max']:.2f}"
            )

            if zero_comp_count > 0 or null_comp_count > 0:
                logger.warning(
                    f"Found {zero_comp_count + null_comp_count} terminated employees with missing compensation data!"
                )
            else:
    ...
    if (
        "employee_tenure" in yearly_snapshot.columns
        and "employee_tenure_band" in yearly_snapshot.columns
    ):
        ...
        if not terminated_employees.empty:
            ...
            logger.info(
                f"  - Tenure stats: mean={tenure_stats['mean']:.3f}, min={tenure_stats['min']:.3f}, max={tenure_stats['max']:.3f}"
            )
            ...
            logger.info(
                f"  - Terminated tenure stats: mean={terminated_tenure_stats['mean']:.3f}, min={terminated_tenure_stats['min']:.3f}, max={terminated_tenure_stats['max']:.3f}"
            )
            ...
            if terminated_null_tenure > 0:
                logger.warning(
                    f"Found {terminated_null_tenure} terminated employees with missing tenure data!"
                )
            else:
    ...
    logger.debug(
        f"Applying age and age band calculations for year {simulation_year} as of {as_of_date_for_age}"
    )
    ...
    if EMP_BIRTH_DATE in yearly_snapshot.columns:
        ...
        logger.debug(
            f"Age calculations completed. Sample ages: {yearly_snapshot[EMP_AGE].head().tolist()}"
        )
        logger.debug(f"Sample age bands: {yearly_snapshot[EMP_AGE_BAND].head().tolist()}")
    else:
        logger.warning(
            f"Birth date column '{EMP_BIRTH_DATE}' not found in yearly snapshot. Age calculations skipped."
        )
        # Add empty age columns to maintain schema consistency
    ...
    logger.info(
        f"Enhanced yearly snapshot for {simulation_year} contains {len(yearly_snapshot)} employees"
    )
...
def consolidate_snapshots_to_parquet(
    snapshots_dir: Union[str, Path], output_path: Union[str, Path]
) -> None:
    """
    ...
    from cost_model.state.age import apply_age
    from cost_model.state.schema import EMP_AGE, EMP_AGE_BAND, EMP_BIRTH_DATE, EMP_ID
    ...
    for file in parquet_files:
        ...
        age_cols_all_null = (EMP_AGE in df.columns and df[EMP_AGE].isna().all()) or (
            EMP_AGE_BAND in df.columns and df[EMP_AGE_BAND].isna().all()
        )

        if EMP_BIRTH_DATE in df.columns and (age_cols_missing or age_cols_all_null):
            as_of_date_for_age = pd.Timestamp(f"{year}-12-31")
            logger.debug(
                f"Adding age calculations to {file.name} for year {year} (missing: {age_cols_missing}, all_null: {age_cols_all_null})"
            )
            df = apply_age(
                df=df,
                birth_col=EMP_BIRTH_DATE,
    ...
    if all_snapshots:  # Only proceed if we have snapshots to concatenate
        ...
        combined["simulation_year"] = (
            combined["simulation_year"].fillna(combined["year"]).astype(int)
        )
        ...
        logger.info(
            f"simulation_year values: {sorted(combined['simulation_year'].dropna().unique())}"
        )
    else:
