...
def _extract_compensation_for_employee(
    ...
    if employee_events.empty:
        logger.info(
            f"No events found for employee {emp_id}, using default compensation: {DEFAULT_COMPENSATION}"
        )
        ...
        if VALUE_JSON in latest_comp_event and pd.notna(latest_comp_event[VALUE_JSON]):
            try:
                ...
                if "new_compensation" in value_data:
                    ...
                    logger.info(
                        f"Found compensation from EVT_COMP event for employee {emp_id}: {compensation}"
                    )
                    return compensation
            except (json.JSONDecodeError, KeyError, ValueError) as e:
                logger.warning(
                    f"Could not parse compensation from EVT_COMP event for employee {emp_id}: {e}"
                )
        ...
        if VALUE_JSON in hire_event and pd.notna(hire_event[VALUE_JSON]):
            try:
                ...
                if "compensation" in value_data:
                    ...
                    logger.info(
                        f"Found compensation from EVT_HIRE event for employee {emp_id}: {compensation}"
                    )
                    return compensation
            except (json.JSONDecodeError, KeyError, ValueError) as e:
                logger.warning(
                    f"Could not parse compensation from EVT_HIRE event for employee {emp_id}: {e}"
                )
    ...
    if EMP_LEVEL in employee_events.columns:
        ...
        if not levels.empty:
            ...
            if level in level_compensation_map:
                ...
                logger.info(
                    f"Using level-based compensation for employee {emp_id} (level {level}): {compensation}"
                )
                return compensation
...
def consolidate_snapshots_to_parquet(
    ...
    logger.info(
        f"Consolidated {len(snapshot_files)} snapshots into {output_path}: {len(consolidated_df)} total records"
    )
