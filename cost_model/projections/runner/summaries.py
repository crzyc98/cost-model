...
def make_yearly_summaries(
    snapshot: pd.DataFrame, year_events: pd.DataFrame, year: int, start_headcount: int = None
    ...
    try:
        ...
        if EMP_DEFERRAL_RATE in active_snapshot.columns:
            participation_rate = (
                len(active_snapshot[active_snapshot[EMP_DEFERRAL_RATE] > 0])
                / max(1, active_headcount)
        ...
        core_summary = {
            ...
            "total_contributions": float(
                snapshot[EMP_CONTR].sum() if EMP_CONTR in snapshot.columns else 0.0
            ),
            "employer_contributions": float(
                (
                    snapshot[EMPLOYER_CORE_CONTRIB].sum()
                    if EMPLOYER_CORE_CONTRIB in snapshot.columns
                    else 0.0
                )
                + (
                    snapshot[EMPLOYER_MATCH_CONTRIB].sum()
                    if EMPLOYER_MATCH_CONTRIB in snapshot.columns
                    else 0.0
                )
            ),
            ...
            "avg_deferral_rate": float(
                snapshot[EMP_DEFERRAL_RATE].mean() if EMP_DEFERRAL_RATE in snapshot.columns else 0.0
            ),
        ...
        logger.error(
            f"Snapshot head (2): {getattr(snapshot, 'head', lambda _: 'N/A')(2) if hasattr(snapshot, 'head') else 'N/A'}"
        )
        raise
    ...
    if start_headcount is not None:
        ...
        hires_count = (
            len(year_events[year_events[EVENT_TYPE] == EVT_HIRE]) if not year_events.empty else 0
        )
        terms_count = (
            len(year_events[year_events[EVENT_TYPE] == EVT_TERM]) if not year_events.empty else 0
        )
        actives_at_year_start = max(0, actives_at_year_end + terms_count - hires_count)
        logger.warning(
            f"start_headcount not provided, calculated as {actives_at_year_start} based on end + terms - hires"
        )
    ...
    employment_summary = {
        ...
        "by_level": (
            snapshot[EMP_LEVEL].value_counts().to_dict()
            if not snapshot.empty and EMP_LEVEL in snapshot.columns
            else {}
        ),
        "by_tenure_band": (
            snapshot[EMP_TENURE_BAND].value_counts().to_dict()
            if not snapshot.empty and EMP_TENURE_BAND in snapshot.columns
            else {}
        ),
    }
