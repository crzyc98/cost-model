...
class DataFrameValidator:
    ...
    def validate_no_duplicates(df: pd.DataFrame, context: str = "") -> Tuple[bool, List[str]]:
        ...
        if duplicated_mask.any():
            ...
            logger.error(f"Duplicate columns found{' in ' + context if context else ''}: {duplicates}")
        ...
        for col in df.columns:
            if col not in processed:
                ...
                if mask.sum() > 1:
                    ...
                    merged = merge_func([duplicate_cols.iloc[:, i] for i in range(duplicate_cols.shape[1])])
                    result_dict[col] = merged
                    logger.info(f"Merged {mask.sum()} duplicate '{col}' columns{' in ' + context if context else ''}")
                else:
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            if isinstance(result, pd.DataFrame) and require_unique_columns:
                ...
                is_valid, duplicates = validator.validate_no_duplicates(result, context or func.__name__)
                
                if not is_valid:
                    # Auto-fix by deduplicating
                    logger.warning(f"Auto-fixing duplicate columns in {context or func.__name__}")
                    result = validator.deduplicate_columns(result, strategy="keep_first", context=context or func.__name__)
