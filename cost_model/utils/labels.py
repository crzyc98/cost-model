...
try:
    ...
    print("Warning (labels.py): Could not import column constants from .columns. Using string literals.")
...
def label_employment_status(df: pd.DataFrame, sim_year: int) -> pd.DataFrame:
    ...
    if EMP_TERM_DATE not in df.columns:
        logger.warning(f"Column '{EMP_TERM_DATE}' not found. Creating with NaT for status labeling.")
    ...
    condlist = [
        ...
        ((df[EMP_TERM_DATE].isna()) |
         (df[EMP_TERM_DATE] > plan_end_date)) &
        (df[EMP_HIRE_DATE].dt.year == sim_year),
    ...
    logger.debug(f"Assigned employment status labels for year {sim_year}. Distribution:\n{df['employment_status'].value_counts()}")
...
if __name__ == '__main__':
    ...
    test_data = {
        EMP_HIRE_DATE: ['2020-01-15', '2023-05-10', '2024-02-20', '2024-08-01', '2022-11-30'],
