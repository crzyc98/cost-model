...
from typing import (
    List, Tuple, Optional, Sequence, Dict, Any
)
...
from utils.columns import GROSS_COMP, RAW_TO_STD_COLS
...
def build_training_data(
    ...
    for filepath in historical_files:
        df = load_and_clean_census(str(filepath), {"required": [EMP_SSN, EMP_BIRTH_DATE, EMP_HIRE_DATE, GROSS_COMP]})
        if df is None:
        ...
        if prev_df is not None:
            ...
            if dropped:
                logger.debug("Dropped %d rows with missing features in %s", dropped, filepath.name)
