...
def analyze_starting_census():
    ...
    if 'employee_birth_date' in df.columns:
        ...
        df['calculated_age'] = calculate_age_from_birth_date(df['employee_birth_date'], simulation_start)
        ...
        df['age_band'] = df['AgeAtYearStart'].map(assign_age_band).astype(AGE_BAND_CATEGORICAL_DTYPE)
