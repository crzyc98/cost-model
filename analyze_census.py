...
def analyze_census():
    ...
    print(f"Compensation range: ${df['employee_gross_compensation'].min():,.0f} - ${df['employee_gross_compensation'].max():,.0f}")
    print()
    
    print("BY JOB LEVEL:")
    level_stats = df.groupby('employee_level').agg({
        'employee_gross_compensation': ['mean', 'count'],
        'employee_tenure': 'mean'
    }).round(1)
    print(level_stats)
    print()
    ...
    expected_new_hire_avg = sum(base_salaries[level] * pct for level, pct in level_dist.items() if level in base_salaries)
    
    print(f"Current workforce average: ${current_avg:,.0f}")
    print(f"Expected new hire average (at base): ${expected_new_hire_avg:,.0f}")
    print(f"New hire disadvantage: {(expected_new_hire_avg - current_avg) / current_avg * 100:.1f}%")
    ...
    if expected_new_hire_avg < current_avg:
        ...
        print(f"This creates structural pay deflation as low-paid new hires replace higher-paid departing employees")
