...
def main():
    ...
    df = pd.read_parquet('validation_run_production_fixed/config_101_production_validation_fixed_summary_statistics.parquet')
    
    print('=== PRODUCTION VALIDATION RESULTS FOR CONFIG_101 ===')
    print()
    print('SUMMARY STATISTICS BY YEAR:')
    print(df[['year', 'active_headcount', 'avg_compensation', 'total_terminations', 'new_hires']].to_string(index=False))
    print()
    ...
    try:
        snapshots = pd.read_parquet('validation_run_production_fixed/consolidated_snapshots.parquet')
        ...
        if not final_snapshot.empty:
            ...
            print(f'Average Compensation: ${final_snapshot["employee_gross_compensation"].mean():.0f}')
            print()
            ...
            if not active_employees.empty:
                ...
                if 'employee_age_band' in active_employees.columns:
                    age_dist = active_employees['employee_age_band'].value_counts(normalize=True).sort_index()
                    print('Age Distribution:')
                    for age_band, pct in age_dist.items():
                        print(f'  {age_band}: {pct:.1%}')
                else:
                    # Fallback to manual binning
                    age_bins = pd.cut(active_employees['employee_age'], bins=[0, 30, 40, 50, 60, 65, 100], labels=['<30', '30-39', '40-49', '50-59', '60-65', '65+'])
                ...
                if 'employee_tenure_band' in active_employees.columns:
                    tenure_dist = active_employees['employee_tenure_band'].value_counts(normalize=True).sort_index()
                    print('Tenure Distribution:')
                    for tenure_band, pct in tenure_dist.items():
                        print(f'  {tenure_band}: {pct:.1%}')
                else:
                    # Fallback to manual binning
                    tenure_bins = pd.cut(active_employees['employee_tenure'], bins=[0, 1, 3, 5, 10, 15, 100], labels=['<1', '1-3', '3-5', '5-10', '10-15', '15+'])
