...
def extract_key_parameters(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    key_params = {
        ...
        # Termination
        "base_rate_for_new_hire": global_params.get("termination_hazard", {}).get(
            "base_rate_for_new_hire"
        ),
        "term_age_mult_under_30": global_params.get("termination_hazard", {})
        .get("age_multipliers", {})
        .get("<30"),
        "term_tenure_mult_under_1": global_params.get("termination_hazard", {})
        .get("tenure_multipliers", {})
        .get("<1"),
        # Compensation
...
def print_analysis_report(results: List[Dict[str, Any]]):
    ...
    print(
        f"Score range: {best_result.get('score', 0):.4f} - {results_sorted[-1].get('score', 0):.4f}"
    )
    ...
    print(
        f"  Headcount Growth: {perf['hc_growth']['achieved']:.1%} vs {perf['hc_growth']['target']:.1%} target"
    )
    print(
        f"    → Error: {perf['hc_growth']['error']:.1%} ({'MISS' if perf['hc_growth']['error'] > 0.005 else 'OK'})"
    )

    print(
        f"  Pay Growth: {perf['pay_growth']['achieved']:.1%} vs {perf['pay_growth']['target']:.1%} target"
    )
    print(
        f"    → Error: {perf['pay_growth']['error']:.1%} ({'OVERSHOOT' if perf['pay_growth']['achieved'] > perf['pay_growth']['target'] else 'OK'})"
    )
    ...
    if age_dist:
        ...
        for band in ["<30", "30-39", "40-49", "50-59", "60-65", "65+"]:
            ...
            print(
                f"  {band:>5}: {achieved:.1%} vs {target:.1%} target → {error:.1%} error ({status})"
            )
...
def main():
    ...
    parser.add_argument(
        "--results-file",
        type=str,
        default="test_tuning/tuning_results.json",
