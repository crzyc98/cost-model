...
PRECISION_SEARCH_SPACE_V2 = {
    ...
    "global_parameters.annual_compensation_increase_rate": [0.032, 0.034, 0.036],  # Higher increases
    ...
    "global_parameters.termination_hazard.base_rate_for_new_hire": [0.035, 0.038, 0.040],  # Lower terms
        ...
        if metrics_file.exists():
            ...
            if len(df) > 1:
                ...
                return {
                    ...
                    "within_10bp": (abs(hc_growth - TARGET_HC_GROWTH) <= 0.001 and 
                                   abs(pay_growth - TARGET_PAY_GROWTH) <= 0.001)
                }
        return None
...
def main():
    ...
    for i, config_path in enumerate(config_paths):
        ...
        if summary:
            print(f"  HC Growth: {summary['hc_growth']:.3%} ({summary['hc_diff_bp']:.0f}bp from target)")
            print(f"  Pay Growth: {summary['pay_growth']:.3%} ({summary['pay_diff_bp']:.0f}bp from target)")
