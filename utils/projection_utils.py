...
from utils.sandbox_utils import generate_new_ssn, generate_new_hires
from utils.ml_logic import prepare_features_for_model, calculate_turnover_score_rule_based, ML_LIBS_AVAILABLE, apply_stochastic_termination, apply_terminations, apply_rule_based_turnover
...
def project_census(
    ...
    comp_increase_rate = scenario_config.get('comp_increase_rate', scenario_config.get('annual_compensation_increase_rate', 0.0))  # annual comp increase rate
    hire_rate = scenario_config.get('hire_rate', scenario_config.get('annual_growth_rate', 0.0))  # Gross hire rate
    termination_rate = scenario_config.get('termination_rate', scenario_config.get('annual_termination_rate', 0.0))  # Target gross term rate
    ...
    for year_num in range(1, projection_years + 1):
        ...
        if needed_hires > 0:
            ...
            # Simulate terminations for new hires within the same year
            termination_rate = scenario_config['termination_rate']
            days_until_end = (year_end_date - new_hires_df['hire_date']).dt.days + 1
            random_offsets = [np.random.randint(0, int(d)) if d > 0 else 0 for d in days_until_end]
            rand_td_series = pd.Series(pd.to_timedelta(random_offsets, unit='D'), index=new_hires_df.index)
            terminate_mask = np.random.rand(len(new_hires_df)) < termination_rate
            new_hires_df.loc[terminate_mask, 'termination_date'] = new_hires_df.loc[terminate_mask, 'hire_date'] + rand_td_series[terminate_mask]
            new_hires_df.loc[terminate_mask, 'status'] = 'Terminated'
        ...
        # Prepare snapshot for classification and boolean flags
        year_snapshot = current_df.copy()
        ...
        # Add boolean flags for new hires and terminations
        year_snapshot['is_new_hire'] = (year_snapshot['hire_date'] >= year_start_date) & (year_snapshot['hire_date'] <= year_end_date)
        year_snapshot['is_terminated'] = (year_snapshot['termination_date'].notna()) & (year_snapshot['termination_date'] >= year_start_date) & (year_snapshot['termination_date'] <= year_end_date)
        year_snapshot['is_new_hire_terminated'] = year_snapshot['is_new_hire'] & year_snapshot['is_terminated']
        # Debug count of new hires flagged
        print(f"DEBUG: Year {current_sim_year} - Flagged new hires: {year_snapshot['is_new_hire'].sum()}")
        # Debug count of new-hire terminations flagged
        print(f"DEBUG: Year {current_sim_year} - Flagged new-hire terminations: {year_snapshot['is_new_hire_terminated'].sum()}")
