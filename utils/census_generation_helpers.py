...
from typing import Set, Generator, Dict, List, Optional, Union # Added Union
...
def generate_employee_record(
    ...
    age = truncnorm.rvs(age_a, age_b, loc=age_mean, scale=safe_age_std_dev, random_state=rng) if age_std_dev > 0 else age_mean
    ...
    try:
        ...
        logger.warning(f"Generated invalid birth date {birth_year}-{birth_month}-{birth_day}, using Jan 1.")
    ...
    if is_new_hire:
        ...
        if hire_start_bound > plan_end_date:
            logger.warning(f"Min possible hire date {hire_start_bound.date()} is after plan end {plan_end_date.date()} for generated age {age}. Setting hire date to plan end date.")
            ...
            hire_date = hire_start_bound + timedelta(days=int(hire_offset)) # Convert hire_offset to int
        ...
        if effective_max_tenure <= 0:
            ...
            tenure_years = truncnorm.rvs(tenure_a, tenure_b, loc=tenure_mean_years, scale=safe_tenure_std_dev, random_state=rng) if tenure_std_dev_years > 0 else tenure_mean_years
    ...
    comp_defaults = {
        'comp_base_salary': 50000, 'comp_increase_per_age_year': 300,
        'comp_increase_per_tenure_year': 500, 'comp_log_mean_factor': 1.0,
        'comp_spread_sigma': 0.20, 'comp_min_salary': 28000
    }
    current_role_params = comp_defaults.copy()
    current_role_params.update(role_params) # Overwrite defaults with role specifics

    target_comp = (current_role_params['comp_base_salary'] +
                   (age_experience_years * current_role_params['comp_increase_per_age_year']) +
                   (record['internal_tenure_at_year_end'] * current_role_params['comp_increase_per_tenure_year']))
...
def calculate_derived_fields(
    ...
    if limits_for_year is None:
        ...
        logger.warning(f"IRS limits for year {year} not found. Using limits from {latest_known_year}.")
    ...
    df_calc['calc_service_start'] = df_calc[[EMP_HIRE_DATE, 'calc_plan_start_date']].max(axis=1, skipna=False) # Propagate NaT if hire date is NaT
    df_calc.loc[df_calc[EMP_HIRE_DATE].isna(), 'calc_service_start'] = pd.NaT # Explicitly set NaT if hire date missing

    df_calc['calc_service_end'] = df_calc[['calc_effective_term_date', 'calc_plan_end_date']].min(axis=1, skipna=False)

    # Calculate days worked in the plan year, handle NaT dates
    valid_service_dates = df_calc['calc_service_start'].notna() & df_calc['calc_service_end'].notna()
    df_calc['calc_days_in_plan_year'] = 0 # Default to 0
    df_calc.loc[valid_service_dates, 'calc_days_in_plan_year'] = \
        (df_calc.loc[valid_service_dates, 'calc_service_end'] - df_calc.loc[valid_service_dates, 'calc_service_start']).dt.days + 1
    df_calc['calc_days_in_plan_year'] = df_calc['calc_days_in_plan_year'].clip(lower=0) # Ensure non-negative
    ...
    df_calc[EMP_PLAN_YEAR_COMP] = (df_calc[EMP_GROSS_COMP] * df_calc['calc_proration_factor']).round(2)
    ...
    df_calc.loc[valid_birth_date_mask, 'calc_age_at_year_end'] = df_calc.loc[valid_birth_date_mask].apply(
        lambda row: (plan_end_date.year - row[EMP_BIRTH_DATE].year -
    ...
    df_calc[EMP_DEFERRAL_RATE] = pd.to_numeric(df_calc[EMP_DEFERRAL_RATE], errors='coerce').fillna(0.0)

    # Calculate deferral based on Plan Year Comp, apply IRS limits
    df_calc['calc_potential_deferral'] = (df_calc[EMP_PLAN_YEAR_COMP] * (df_calc[EMP_DEFERRAL_RATE] / 100.0)).round(2)
    df_calc[EMP_CONTR] = df_calc['calc_potential_deferral'].clip(lower=0, upper=max_deferral) # Apply limits

    # NEC
    df_calc[EMPLOYER_CORE] = (df_calc[EMP_PLAN_YEAR_COMP] * nec_rate).round(2).clip(lower=0) # Ensure non-negative
    ...
    df_calc[EMPLOYER_MATCH] = (actual_deferral_for_match * match_rate).round(2).clip(lower=0) # Ensure non-negative
...
def select_weighted_terminations(
    ...
    if num_to_terminate >= len(eligible_df):
        logger.warning(f"Requesting {num_to_terminate} terminations, but only {len(eligible_df)} eligible. Terminating all.")
    ...
    # Tenure Risk
    if 'tenure' not in eligible_df.columns: eligible_df['tenure'] = 0.0 # Add temp if missing
    eligible_df['low_tenure'] = pd.to_numeric(eligible_df['tenure'], errors='coerce').fillna(0.0) < LOW_TENURE_THRESHOLD
    ...
    if comp_col in eligible_df.columns and pd.api.types.is_numeric_dtype(eligible_df[comp_col]):
        if role_col in eligible_df.columns and eligible_df[role_col].nunique() > 1:
            try:
                ...
                logger.warning(f"Error calculating comp percentile by role: {e}. Falling back to global.")
    ...
    if total_weight > 0 and np.isfinite(total_weight):
        ...
        if np.any(~np.isfinite(probabilities)) or np.isclose(probabilities.sum(), 0):
            ...
            logger.warning("Termination probabilities sum to %.4f. Renormalizing.", probabilities.sum())
    ...
    try:
        ...
        logger.warning(f"Error during weighted choice: {e}. Falling back to simple random selection.")
        # Fallback to simple random sampling without weights
        return rng.choice(
            eligible_df.index, size=num_to_terminate, replace=False
        )
