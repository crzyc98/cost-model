...
def bootstrap_events_df() -> pd.DataFrame:
    ...
    bootstrap_event_path = Path("output_dev/testing/bootstrap_events_for_snapshot_test.parquet")
    ...
    if not bootstrap_event_path.exists():
         pytest.skip(f"Bootstrap event file not found at {bootstrap_event_path}. Run bootstrap script first.")
...
def test_build_full_structure(bootstrap_events_df):
    ...
    assert actual_cols == expected_cols, f"Snapshot columns mismatch. Expected: {expected_cols}, Got: {actual_cols}"

    # Check dtypes (using the definition from snapshot.py)
    for col, expected_dtype in snapshot.SNAPSHOT_DTYPES.items():
        assert str(snap[col].dtype) == str(expected_dtype), f"Dtype mismatch for {col}. Expected: {expected_dtype}, Got: {snap[col].dtype}"
...
def test_build_full_active_count(bootstrap_events_df):
    ...
    assert snap['active'].sum() == expected_active_count, f"Expected {expected_active_count} active employees."
...
def test_build_full_known_employee(bootstrap_events_df):
    ...
    assert known_active_emp_id in snap.index, f"Employee {known_active_emp_id} not found in snapshot."
    emp_data = snap.loc[known_active_emp_id]

    assert emp_data['active'] == True, f"Employee {known_active_emp_id} should be active"

    # Verify other fields for this known employee
    assert pd.notna(emp_data['hire_date']), f"Employee {known_active_emp_id} should have a hire date"
    assert pd.notna(emp_data['birth_date']), f"Employee {known_active_emp_id} should have a birth date"
    assert pd.notna(emp_data['role']), f"Employee {known_active_emp_id} should have a role"
    assert pd.notna(emp_data['current_comp']), f"Employee {known_active_emp_id} should have current compensation"
    assert pd.isna(emp_data['term_date']), f"Employee {known_active_emp_id} should not have a termination date"
    assert emp_data['hire_date'] == expected_hire_date, f"Employee {known_active_emp_id} hire date mismatch. Expected: {expected_hire_date}, Got: {emp_data['hire_date']}"
    assert emp_data['birth_date'] == expected_birth_date, f"Employee {known_active_emp_id} birth date mismatch. Expected: {expected_birth_date}, Got: {emp_data['birth_date']}"
    assert emp_data['role'] == expected_role, f"Employee {known_active_emp_id} role mismatch. Expected: {expected_role}, Got: {emp_data['role']}"
    assert emp_data['current_comp'] == expected_comp, f"Employee {known_active_emp_id} compensation mismatch. Expected: {expected_comp}, Got: {emp_data['current_comp']}"

# --- Helper to create sample events ---
# Use the helper from event_log.py if available, otherwise define one here
def _create_test_event(emp_id, time_str, type, value_num=None, value_json_dict=None, meta_str=None):
    event_data = {
...
def test_update_new_hire(initial_snapshot):
    ...
    new_events_list = [
        _create_test_event(new_emp_id, "2025-03-15 09:00:00", event_log.EVT_HIRE,
                           value_json_dict={'role': 'Analyst', 'birth_date': '1995-05-20'}),
        _create_test_event(new_emp_id, "2025-03-15 09:00:01", event_log.EVT_COMP, value_num=55000.0)
    ]
    new_events_df = pd.DataFrame(new_events_list)
    # Ensure correct dtypes for the new events DataFrame before passing to update
    new_events_df = new_events_df[event_log.EVENT_COLS].astype(event_log.EVENT_PANDAS_DTYPES)
    ...
    assert new_hire_data['active'] == True
...
def test_update_termination(initial_snapshot):
    ...
    emp_to_term = initial_snapshot[initial_snapshot['active'] == True].index[0]
    ...
    assert term_emp_data['active'] == False, "Terminated employee should be inactive."
...
def test_update_comp_change(initial_snapshot):
    ...
    emp_to_update = initial_snapshot[initial_snapshot['active'] == True].index[1]
    original_comp = prev_snap.loc[emp_to_update, 'current_comp']
    new_comp = original_comp * 1.10 # Example 10% raise

    new_events_list = [
        _create_test_event(emp_to_update, "2025-04-01 00:00:00", event_log.EVT_COMP, value_num=new_comp)
    ]
    new_events_df = pd.DataFrame(new_events_list)
    new_events_df = new_events_df[event_log.EVENT_COLS].astype(event_log.EVENT_PANDAS_DTYPES)
    ...
    assert updated_emp_data['current_comp'] == new_comp, "Compensation should be updated."
    assert updated_emp_data['active'] == True # Should still be active
...
def test_update_multiple_events_same_employee(initial_snapshot):
    ...
    prev_snap = initial_snapshot
    # TODO: Choose an ID known to be active
    emp_to_update = initial_snapshot[initial_snapshot['active'] == True].index[2]

    new_events_list = [
        _create_test_event(emp_to_update, "2025-02-01 09:00:00", event_log.EVT_COMP, value_num=70000.0),
        _create_test_event(emp_to_update, "2025-08-15 10:00:00", event_log.EVT_COMP, value_num=75000.0), # Last comp event
        _create_test_event(emp_to_update, "2025-11-30 17:00:00", event_log.EVT_TERM) # Last term event
    ]
    new_events_df = pd.DataFrame(new_events_list)
    new_events_df = new_events_df[event_log.EVENT_COLS].astype(event_log.EVENT_PANDAS_DTYPES)
    ...
    updated_emp_data = updated_snap.loc[emp_to_update]
    assert updated_emp_data['current_comp'] == 75000.0, "Should reflect the last comp event."
    assert updated_emp_data['term_date'] == pd.Timestamp("2025-11-30 17:00:00"), "Should reflect the last term event."
    assert updated_emp_data['active'] == False

def test_update_empty_events(initial_snapshot):
    """Test update with an empty new_events DataFrame."""
    prev_snap = initial_snapshot
    empty_new_events = pd.DataFrame(columns=event_log.EVENT_COLS).astype(event_log.EVENT_PANDAS_DTYPES)

    updated_snap = snapshot.update(prev_snap, empty_new_events)

    pd.testing.assert_frame_equal(updated_snap, prev_snap), "Snapshot should be unchanged if new_events is empty."
...
def test_update_terminated_employee_again(initial_snapshot):
    ...
    first_term_event = _create_test_event(emp_to_term, "2025-07-31 17:00:00", event_log.EVT_TERM)
    ...
    second_term_event = _create_test_event(emp_to_term, "2025-08-15 17:00:00", event_log.EVT_TERM)
...
def test_update_compensation_for_terminated_employee(initial_snapshot):
    ...
    prev_snap = initial_snapshot
    # Get an active employee
    emp_to_update = initial_snapshot[initial_snapshot['active'] == True].index[0]
    
    # First terminate them
    term_event = _create_test_event(emp_to_update, "2025-07-31 17:00:00", event_log.EVT_TERM)
    ...
    comp_event = _create_test_event(emp_to_update, "2025-08-15 00:00:00", event_log.EVT_COMP, value_num=100000.0)
    ...
    assert emp_data['current_comp'] == snap_after_term.loc[emp_to_update, 'current_comp']
...
def test_update_multiple_new_hires(initial_snapshot):
    ...
    new_hires = [
        _create_test_event("NH_001", "2025-03-15 09:00:00", event_log.EVT_HIRE,
                          value_json_dict={'role': 'Analyst', 'birth_date': '1995-05-20'}),
        _create_test_event("NH_002", "2025-03-15 09:00:00", event_log.EVT_HIRE,
                          value_json_dict={'role': 'Developer', 'birth_date': '1992-08-15'}),
        _create_test_event("NH_003", "2025-03-15 09:00:00", event_log.EVT_HIRE,
                          value_json_dict={'role': 'Manager', 'birth_date': '1985-03-10'})
    ]
    
    # Add compensations for each new hire
    compensations = [
        _create_test_event("NH_001", "2025-03-15 09:00:01", event_log.EVT_COMP, value_num=55000.0),
        _create_test_event("NH_002", "2025-03-15 09:00:01", event_log.EVT_COMP, value_num=70000.0),
        _create_test_event("NH_003", "2025-03-15 09:00:01", event_log.EVT_COMP, value_num=90000.0)
    ]
...
def test_update_invalid_event_data(initial_snapshot):
    ...
    invalid_events = [
        ...
        _create_test_event("NH_001", "2025-03-15 09:00:00", event_log.EVT_HIRE,
                          value_json_dict={'role': 'Analyst'}),  # Missing birth_date
        # Invalid comp event (non-numeric value)
        _create_test_event("NH_002", "2025-03-15 09:00:00", event_log.EVT_COMP,
                          value_num="not_a_number"),
        # Invalid term event (invalid timestamp)
        _create_test_event("NH_003", "2025-03-15 09:00:00", event_log.EVT_TERM,
