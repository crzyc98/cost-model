...
def base_snapshot():
    return (
        pd.DataFrame(
            {
                ...
                "birth_date": pd.to_datetime(
                    ["1980-01-01", "1990-01-01", "1985-01-01"]
                ),
        ...
        .set_index(EMP_ID)
        .astype({"active": pd.BooleanDtype(), "role": pd.StringDtype()})
    )  # Set index and dtypes
        ...
        [
            {
                ...
                "value_json": json.dumps(
                    {"employee_deferral": 5000, "employer_match": 3000}
                ),  # Use JSON string & corrected match
                "meta": None,
            }
        ]
    )
