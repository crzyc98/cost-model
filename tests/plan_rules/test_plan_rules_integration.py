...
def test_full_plan_rules_pipeline(tiny_snapshot, base_events):
    ...
    enrol_cfg = EnrollmentConfig(
        auto_enrollment=AutoEnrollmentConfig(
            enabled=False, default_rate=0.02, window_days=0
        ),
        voluntary_enrollment_rate=1.0,
        default_rate=0.01,
    )
    contr_cfg = ContributionConfig(
        default_rate=0.0, employer_match_tiers=[{"match_rate": 1.0, "cap_pct": 0.01}]
    )
    ...
    assert {"EVT_ELIGIBLE", "EVT_ENROLL", "EVT_CONTRIB", "EVT_AUTO_INCREASE"}.issubset(
        types
    )
