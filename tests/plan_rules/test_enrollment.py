...
def test_already_opted_out():
    ...
    events = make_events([
        {'employee_id': 2, 'event_type': 'EVT_ELIGIBLE', 'event_time': pd.Timestamp('2024-12-01')},
        {'employee_id': 2, 'event_type': 'EVT_OPT_OUT', 'event_time': pd.Timestamp('2024-12-15')},
    ])
    cfg = EnrollmentConfig(window_days=30)
    result = enrollment_run(snap, events, today, cfg)
    assert len(result) == 0
...
def test_already_enrolled():
    ...
    events = make_events([
        {'employee_id': 3, 'event_type': 'EVT_ELIGIBLE', 'event_time': pd.Timestamp('2024-12-01')},
        {'employee_id': 3, 'event_type': 'EVT_ENROLL', 'event_time': pd.Timestamp('2024-12-20')},
    ])
    cfg = EnrollmentConfig(window_days=30)
    result = enrollment_run(snap, events, today, cfg)
    assert len(result) == 0

# --- New test for auto and voluntary enrollment ---
import numpy as np
...
def base_events():
    ...
    return pd.DataFrame([
        {'event_time': datetime(2025,1,1),'employee_id':'A','event_type':'EVT_ELIGIBLE'},
        {'event_time': datetime(2025,1,1),'employee_id':'B','event_type':'EVT_ELIGIBLE'},
        {'event_time': datetime(2025,1,1),'employee_id':'C','event_type':'EVT_ELIGIBLE'},
        {'event_time': datetime(2025,1,1),'employee_id':'D','event_type':'EVT_ELIGIBLE'},
...
def test_auto_and_voluntary_enrollment():
    cfg = EnrollmentConfig(
      auto_enrollment=AutoEnrollmentConfig(enabled=True, default_rate=0.03, window_days=90),
...
def test_auto_and_voluntary_match_multiplier():
    ...
    class DummyConfig:
        auto_enrollment = AutoEnrollmentConfig(enabled=True, default_rate=0.05, window_days=0)
...
def test_rng_seed_override():
    ...
    events_df = pd.DataFrame([
        {'employee_id': str(i), 'event_type': 'EVT_ELIGIBLE', 'event_time': pd.Timestamp('2025-01-01')}
        for i in range(10)
    ])
    ...
    assert df1.equals(df2), "Voluntary enrollment should be deterministic if random_seed is set"
...
def test_match_incentive_effect():
    ...
    events = pd.DataFrame([
        {'employee_id': emp, 'event_type': 'EVT_ELIGIBLE', 'event_time': pd.Timestamp('2025-01-01')}
...
def test_eligible_not_opted_out_or_enrolled():
    ...
    events = make_events([
        {'employee_id': 4, 'event_type': 'EVT_ELIGIBLE', 'event_time': pd.Timestamp('2024-12-01')},
    ])
