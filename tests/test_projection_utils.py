...
def test_apply_turnover_no_terminations(monkeypatch):
    ...
    out = _apply_turnover(df, 'hire_date', 0.5, pd.Timestamp('2021-01-01'), pd.Timestamp('2021-12-31'), rng, sampler, prev_salaries)
    # no changes
...
def test_comp_bump_groups():
    ...
    out = _apply_comp_bump(df.copy(), 'gross', {}, rate=0.1, rng=rng, sampler=StubSampler())
    # tenure==0 unchanged
