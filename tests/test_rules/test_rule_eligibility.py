...
def test_apply_and_agent_helper_consistency():
    ...
    eligibility_config = {'min_age': 21, 'min_service_months': 12, 'min_hours_worked': 40}
    ...
    for idx, row in df.iterrows():
        expected = agent_is_eligible(
            row['birth_date'], row['hire_date'], row['status'], row['hours_worked'],
            eligibility_config, sim_end
        )
