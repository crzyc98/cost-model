...
def test_parse_match_tiers_single_tier():
    ...
    assert tiers[0] == {'match_pct': pytest.approx(1.0), 'deferral_cap_pct': pytest.approx(0.03)}
...
def test_parse_match_tiers_partial_invalid():
    ...
    assert len(tiers) == 1
    assert tiers[0] == {'match_pct': pytest.approx(0.75), 'deferral_cap_pct': pytest.approx(0.04)}
