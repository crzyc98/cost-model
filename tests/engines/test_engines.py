...
def make_mini_snapshot():
    ...
    return pd.DataFrame([
        {'employee_id': 'E1', 'role': 'A', 'tenure_band': '0-1', 'active': True, 'term_date': pd.NaT, 'current_compensation': 50000},
        {'employee_id': 'E2', 'role': 'B', 'tenure_band': '1-3', 'active': True, 'term_date': pd.NaT, 'current_compensation': 60000},
        {'employee_id': 'E3', 'role': 'C', 'tenure_band': '3+', 'active': True, 'term_date': pd.NaT, 'current_compensation': 70000},
...
def test_comp_bump_interface():
    ...
    assert all(abs(a-b) < 1e-4 for a, b in zip(sorted(df['value_num']), sorted(expected)))
...
def test_term_run_interface():
    ...
    for t in df['event_time']:
        assert pd.Timestamp('2025-01-01') <= pd.Timestamp(t) <= pd.Timestamp('2025-12-31')
